...
`;let ft=class extends at{constructor(){super(...arguments),this._cards=[]}setConfig(t){this._config=t,this._createCards()}set hass(t){this._hass=t,this._cards.forEach(e=>{void 0!==e.hass&&(e.hass=t)})}get hass(){return this._hass}_createCards(){this._config.cards&&(this._cards=this._config.cards.map(t=>{const e=t.type?.startsWith("custom:")?t.type.replace("custom:",""):`hui-${t.type}-card`,i=document.createElement(e);return"function"==typeof i.setConfig&&i.setConfig(t),i}),this.requestUpdate())}render(){return F`
    ...
    `}getCardSize(){return 6}};ft.styles=[ut,r`
      :host {
        display: block;
        min-height: 100vh;
        position: relative;
    ...
    `}}t([pt({attribute:!1})],mt.prototype,"hass",void 0),t([pt({attribute:!1})],mt.prototype,"_config",void 0);let bt=class extends mt{get _buttonConfig(){return this._config}setConfig(t){super.setConfig(t),t.entity&&this.setTrackedEntities([t.entity])}_handleTap(){const t=this._buttonConfig.tap_action?.action??"toggle";"toggle"===t&&this._config.entity?this.toggle(this._config.entity):"navigate"===t&&this._buttonConfig.tap_action?.navigation_path&&(window.location.hash=this._buttonConfig.tap_action.navigation_path)}render(){if(!this.hass||!this._config)return F``;const t=this._config.entity?this.getEntity(this._config.entity):void 0,e=!!t&&this.isOn(this._config.entity),i=this._config.name??t?.attributes.friendly_name??"",s=this._config.icon??t?.attributes.icon??"mdi:help-circle";let n="";if(!1!==this._buttonConfig.show_state&&t){const e=t.attributes.unit_of_measurement;n=e?`${t.state} ${e}`:"on"===t.state?"Pa":"off"===t.state?"Av":t.state}return F`
      <div class="glass button ${e?"active":""}" @click=${this._handleTap}>
        ...
        <div class="info">
          ...
          ${n?F`<div class="state">${n}</div>`:""}
        </div>
      </div>
    `}};bt.styles=[mt.glassStyles,r`
      :host { display: block; }
      .button {
        display: flex;
        align-items: center;
    ...
    `],vt=t([lt("glass-chip")],vt);let _t=class extends mt{get _headerConfig(){return this._config}setConfig(t){super.setConfig(t);const e=[];t.weather_entity&&e.push(t.weather_entity),t.chips&&t.chips.forEach(t=>{t.entity&&e.push(t.entity)}),this.setTrackedEntities(e)}_renderChip(t){const e=this.getEntity(t.entity);if(!e)return F``;let i=t.icon??"",s="",n=!1;switch(t.chip_type){case"person":i=i||"mdi:account";s=`${e.attributes.friendly_name??""} · ${"home"===e.state?"Hemma":"Borta"}`,n="home"===e.state;break;case"battery":i=i||"mdi:cellphone",s=`${e.state} %`,n=Number(e.state)>20;break;case"lights":i=i||"mdi:lightbulb-group",s=`${e.state} st`,n=Number(e.state)>0;break;default:i=i||"mdi:information",s=e.state}return F`
      <div class="chip ${n?"active":""}">
        <ha-icon .icon=${i}></ha-icon>
        <span>${s}</span>
      </div>
    `}render(){if(!this.hass||!this._config)return F``;const t=this.hass.user?.name??"",e=!1!==this._headerConfig.greeting?function(t){const e=(new Date).getHours();return e>=5&&e<10?`God morgon, ${t}`:e>=10&&e<17?`Hej, ${t}`:e>=17&&e<22?`God kvall, ${t}`:`God natt, ${t}`}(t):t,i=this._headerConfig.weather_entity?this.getEntity(this._headerConfig.weather_entity):void 0,s=i?.state??"",n=i?.attributes.temperature??"",o=i?.attributes.temperature_unit??"°C",r={"clear-night":"mdi:weather-night",cloudy:"mdi:weather-cloudy",fog:"mdi:weather-fog",hail:"mdi:weather-hail",lightning:"mdi:weather-lightning","lightning-rainy":"mdi:weather-lightning-rainy",partlycloudy:"mdi:weather-partly-cloudy",pouring:"mdi:weather-pouring",rainy:"mdi:weather-rainy",snowy:"mdi:weather-snowy","snowy-rainy":"mdi:weather-snowy-rainy",sunny:"mdi:weather-sunny",windy:"mdi:weather-windy","windy-variant":"mdi:weather-windy-variant",exceptional:"mdi:alert-circle-outline"}[s]??"mdi:weather-cloudy";return F`
      ...
      <div class="glass room-card ${i?"active":""}" @click=${this._handleCardTap}>
        ...
        ${t.length?F`
          <div class="sub-buttons">
            ${t.map(t=>{const e=this.isOn(t.entity),i=t.icon??this.getEntity(t.entity)?.attributes.icon??"mdi:lightbulb";return F`
                <div
                  class="sub-btn ${e?"on":""}"
    ...
    `],yt=t([lt("glass-room-card")],yt);let xt=class extends mt{constructor(){super(...arguments),this._dragging=!1,this._dragValue=0}setConfig(t){super.setConfig(t),t.entity&&this.setTrackedEntities([t.entity])}_handleSliderInteraction(t){if(!this._config.entity)return;const e=this.getEntity(this._config.entity);if(!e||"off"===e.state)return void this.callService("light","turn_on",{brightness_pct:100},this._config.entity);const i=t.currentTarget.getBoundingClientRect(),s=t=>{const e=Math.max(0,Math.min(t-i.left,i.width)),s=Math.round(e/i.width*100);this._dragValue=Math.max(1,Math.min(100,s))},n="touches"in t?t.touches[0].clientX:t.clientX;s(n),this._dragging=!0;const o=t=>{const e="touches"in t?t.touches[0].clientX:t.clientX;s(e)},r=()=>{this._dragging=!1,this.callService("light","turn_on",{brightness_pct:this._dragValue},this._config.entity),document.removeEventListener("mousemove",o),document.removeEventListener("mouseup",r),document.removeEventListener("touchmove",o),document.removeEventListener("touchend",r)};document.addEventListener("mousemove",o),document.addEventListener("mouseup",r),document.addEventListener("touchmove",o,{passive:!0}),document.addEventListener("touchend",r)}render(){if(!this.hass||!this._config?.entity)return F``;const t=this.getEntity(this._config.entity);if(!t)return F``;const e="on"===t.state,i=this._dragging?this._dragValue:function(t){if(!t||"on"!==t.state)return 0;const e=t.attributes.brightness;return e?Math.round(e/255*100):100}(t),s=this._config.name??t.attributes.friendly_name??"",n=this._config.icon??t.attributes.icon??"mdi:lightbulb";return F`
        ...
        <div class="panel">
          ...
          ${this._config.title?F`
            <div class="popup-header">
              ${this._config.icon?F`<ha-icon .icon=${this._config.icon}></ha-icon>`:""}
    ...
    `:F``}getCardSize(){return 0}};$t.styles=[ut,r`
      :host { display: block; }
      .overlay {
        position: fixed;
        inset: 0;
    ...
    `],t([pt({attribute:!1})],$t.prototype,"_config",void 0),t([gt()],$t.prototype,"_isOpen",void 0),t([gt()],$t.prototype,"_isClosing",void 0),$t=t([lt("glass-popup")],$t);let wt=class extends at{constructor(){super(...arguments),this._activeHash="",this._onHashChange=()=>{const t=window.location.hash.replace("#","");this._config.items.some(e=>e.hash===t)&&(this._activeHash=t)}}connectedCallback(){super.connectedCallback(),this._activeHash=window.location.hash.replace("#","")||this._config?.items?.[0]?.hash||"",window.addEventListener("hashchange",this._onHashChange)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("hashchange",this._onHashChange)}setConfig(t){if(!t.items?.length)throw new Error('glass-nav-bar requires "items"');this._config=t}_handleTap(t){this._activeHash=t,window.location.hash=t}render(){return this._config?.items?F`
      <div class="nav-bar">
        ${this._config.items.map(t=>F`
    ...
    `:F``}getCardSize(){return 0}};wt.styles=r`
    :host {
      display: block;
      position: fixed;
      bottom: 0;
  ...
  `,t([pt({attribute:!1})],wt.prototype,"hass",void 0),t([pt({attribute:!1})],wt.prototype,"_config",void 0),t([gt()],wt.prototype,"_activeHash",void 0),wt=t([lt("glass-nav-bar")],wt);const kt=window;kt.customCards=kt.customCards||[],kt.customCards.push({type:"glass-background",name:"Glass Background",description:"Animated gradient background"},{type:"glass-button",name:"Glass Button",description:"Toggle/info button"},{type:"glass-chip",name:"Glass Chip",description:"Small status pill"},{type:"glass-header",name:"Glass Header",description:"Greeting, weather, status chips"},{type:"glass-room-card",name:"Glass Room Card",description:"Room with sub-buttons and popup"},{type:"glass-light-slider",name:"Glass Light Slider",description:"Brightness slider with glow"},{type:"glass-popup",name:"Glass Popup",description:"Modal overlay"},{type:"glass-nav-bar",name:"Glass Nav Bar",description:"Bottom navigation"}),console.info("%c GLASS CARDS %c v0.1.0 ","color: white; background: #4FC3F7; font-weight: bold; padding: 2px 6px; border-radius: 4px 0 0 4px;","color: #4FC3F7; background: rgba(79,195,247,0.1); padding: 2px 6px; border-radius: 0 4px 4px 0;");
