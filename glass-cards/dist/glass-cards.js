...
`;let $t=class extends ct{constructor(){super(...arguments),this._isOpen=!1,this._isClosing=!1,this._cards=[],this._onHashChange=()=>{this._checkHash()}}connectedCallback(){super.connectedCallback(),window.addEventListener("hashchange",this._onHashChange),this._checkHash()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("hashchange",this._onHashChange)}_checkHash(){if(!this._config?.hash)return;const t=window.location.hash.replace("#","");t!==this._config.hash||this._isOpen?t!==this._config.hash&&this._isOpen&&this._close():this._open()}_open(){this._isOpen=!0,this._isClosing=!1,this._createPopupCards()}_close(){this._isClosing=!0,setTimeout(()=>{this._isOpen=!1,this._isClosing=!1},350),window.location.hash.replace("#","")===this._config.hash&&history.replaceState(null,"",window.location.pathname+window.location.search)}_handleBackdropClick(){this._close()}setConfig(t){if(!t.hash)throw new Error('glass-popup requires a "hash" property');this._config=t}set hass(t){this._hass=t,this._cards.forEach(e=>{e.hass=t})}get hass(){return this._hass}_createPopupCards(){this._config?.cards&&(this._cards=this._config.cards.map(t=>{const e=t.type?.startsWith("custom:")?t.type.replace("custom:",""):`hui-${t.type}-card`,i=document.createElement(e);return"function"==typeof i.setConfig&&i.setConfig(t),this.hass&&(i.hass=this.hass),i}),this.requestUpdate())}render(){return this._isOpen||this._isClosing?V`
  ...
  `,t([gt({attribute:!1})],Et.prototype,"hass",void 0),t([gt({attribute:!1})],Et.prototype,"_config",void 0),t([bt()],Et.prototype,"_activeHash",void 0),Et=t([dt("glass-nav-bar")],Et);let Ct=class extends mt{get _vacuumConfig(){return this._config}setConfig(t){super.setConfig(t),t.entity&&this.setTrackedEntities([t.entity])}_getStatusText(t){return{cleaning:"Stader",docked:"Dockad",paused:"Pausad",returning:"Atergar",idle:"Inaktiv",error:"Fel",unavailable:"Otillganglig"}[t]??t}_start(){this._config.entity&&this.callService("vacuum","start",void 0,this._config.entity)}_stop(){this._config.entity&&this.callService("vacuum","return_to_base",void 0,this._config.entity)}_cleanRoom(t){this._config.entity&&null!=t.room_id&&this.callService("vacuum","send_command",{command:"app_segment_clean",params:[t.room_id]},this._config.entity)}render(){if(!this.hass||!this._config?.entity)return V``;const t=this.getEntity(this._config.entity);if(!t)return V``;const e=t.state,i="cleaning"===e,a="error"===e,s=t.attributes.battery_level,r=this._config.name??t.attributes.friendly_name??"Vacuum",o=this._config.icon??"mdi:robot-vacuum";return V`
    ...
    `],St=t([dt("glass-info-row")],St);let Mt=class extends ct{set hass(t){}setConfig(t){if(!t.label)throw new Error('glass-section requires a "label" property');this._config=t}render(){return this._config?V`
      <div class="section">
        ${this._config.icon?V`<ha-icon .icon=${this._config.icon}></ha-icon>`:""}
        ...
        ${0===i.length?V`<div class="empty-state">Inga avgångar</div>`:V`
              <div class="departure-list">
                ${i.map(t=>V`
                    ...
                    ${t.deviations?.length?t.deviations.filter(t=>t.message).map(t=>V`<div class="deviation">${t.message}</div>`):X}
                  `)}
              </div>
            `}
      </div>
...
`,Nt="glass-hub-theme";const Pt="glass-hub-weather-bg";function zt(){return"off"!==localStorage.getItem(Pt)}function Ft(t){localStorage.setItem(Pt,t?"on":"off")}let Lt=null;function jt(){return Lt}function Dt(t,e=8){return Math.abs(t)>e}const Bt=t=>q`
...
`,Ot={lamp:Bt(q`
...
</svg>`,ve={sunny:{day:ae,night:se},"clear-night":{day:se},partlycloudy:{day:oe,night:ne},cloudy:{day:re},rainy:{day:le},pouring:{day:ce},snowy:{day:he},"snowy-rainy":{day:de},lightning:{day:pe},"lightning-rainy":{day:ue},fog:{day:ge},hail:{day:q`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
  <path d="M6.5 14.5a4 4 0 0 1-.4-8 5.2 5.2 0 0 1 10.1-1.2 3.9 3.9 0 0 1 1.3 7.6 4 4 0 0 1-1.5.3z"></path>
  <path d="M8 18v.01M12 18v.01M16 18v.01M10 21v.01M14 21v.01" stroke-width="2.6"></path>
    ...
    `],t([gt({attribute:!1})],ke.prototype,"config",void 0),customElements.define("hub-lighting-tile",ke);class $e extends mt{constructor(){super(...arguments),this._open=()=>{this.dispatchEvent(new CustomEvent("hub-car-open",{bubbles:!0,composed:!0}))}}render(){if(!this.hass||!this.config?.volvo)return V``;const t=this.config.volvo,e=t.battery_entity?this.getEntity(t.battery_entity)?.state:void 0,i=t.range_entity?this.getEntity(t.range_entity)?.state:void 0,a=t.lock_entity?this.getEntity(t.lock_entity)?.state:void 0,s="charging"===(t.charging_entity?this.getEntity(t.charging_entity)?.state:void 0),r=[e&&!Number.isNaN(Number(e))?`${e}%`:null,i&&!Number.isNaN(Number(i))?`${i} km`:null,s?"Laddar":null].filter(Boolean);return V`
      <div class="card" role="button" tabindex="0" aria-label="Visa bilen" @click=${this._open}>
        <div class="top-row">
          <span class="ic">${Ot.car}</span>
          ${"locked"===a||"unlocked"===a?V`<span class="lock ${"locked"===a?"":"unlocked"}">
                ${"locked"===a?"Låst":"Olåst"}
              </span>`:X}
        </div>
        <div>
    ...
    `}}function Se(t){const e=t??[];return{open:e.filter(t=>"needs_action"===t.status),done:e.filter(t=>"completed"===t.status)}}async function Me(t,e){try{const i=await t.callWS({type:"call_service",domain:"todo",service:"get_items",service_data:{},target:{entity_id:e},return_response:!0});return i?.response?.[e]?.items??[]}catch{return null}}Ce.styles=[Tt,o`
      :host { display: block; height: 100%; }
      .card {
        box-sizing: border-box; height: 100%;
        display: flex; flex-direction: column; justify-content: space-between; gap: 10px;
    ...
    `],t([gt({attribute:!1})],Ce.prototype,"config",void 0),customElements.define("hub-vacuum-card",Ce);class Ae extends mt{constructor(){super(...arguments),this._items=null,this._lastCount="",this._fetchSeq=0,this._open=()=>{this.dispatchEvent(new CustomEvent("hub-todo-open",{bubbles:!0,composed:!0}))}}updated(t){super.updated(t);const e=this.config?.todo_entity;if(!e||!this.hass)return;const i=this.getEntity(e)?.state??"";i!==this._lastCount&&(this._lastCount=i,this._refresh())}async _refresh(){if(!this.hass||!this.config?.todo_entity)return;const t=++this._fetchSeq,e=await Me(this.hass,this.config.todo_entity);t===this._fetchSeq&&(this._items=e)}_complete(t,e){t.stopPropagation(),this.config.todo_entity&&this.callService("todo","update_item",{item:e.uid,status:"completed"},this.config.todo_entity)}render(){if(!this.hass||!this.config?.todo_entity)return V``;const{open:t}=Se(this._items);return V`
      <div class="card" role="button" tabindex="0" aria-label="Visa att göra-listan" @click=${this._open}>
        ...
        ${0===t.length?V`<span class="empty">Inget att göra</span>`:t.slice(0,4).map(t=>V`
              ...
              `)}
        ${t.length>4?V`<span class="more">+${t.length-4} till</span>`:X}
      </div>
    `}}Ae.styles=[Tt,o`
      :host { display: block; height: 100%; }
      .card {
    ...
    `],t([gt({attribute:!1})],Ae.prototype,"config",void 0),t([bt()],Ae.prototype,"_items",void 0),customElements.define("hub-todo-card",Ae);const Te=new Set(["off","unavailable","unknown","standby","idle"]);function Ne(t,e){for(const i of e){const e=t[i.entity];if(e&&"playing"===e.state)return{entity:e,name:i.name}}for(const i of e){const e=t[i.entity];if(e&&!Te.has(e.state))return{entity:e,name:i.name}}return null}function Pe(t,e){if(!t)return 0;const i=t.attributes,a="number"==typeof i.media_duration?i.media_duration:0;if(a<=0)return 0;let s="number"==typeof i.media_position?i.media_position:0;const r="string"==typeof i.media_position_updated_at?Date.parse(i.media_position_updated_at):NaN;return"playing"!==t.state||Number.isNaN(r)||(s+=(e-r)/1e3),Math.max(0,Math.min(100,s/a*100))}class ze extends mt{constructor(){super(...arguments),this.players=[],this._now=Date.now()}connectedCallback(){super.connectedCallback(),this._interval=window.setInterval(()=>{this._now=Date.now()},1e3)}disconnectedCallback(){super.disconnectedCallback(),void 0!==this._interval&&(clearInterval(this._interval),this._interval=void 0)}_goto(){this.dispatchEvent(new CustomEvent("hub-goto-page",{detail:{page:"media"},bubbles:!0,composed:!0}))}_togglePlay(t,e){t.stopPropagation(),this.callService("media_player","media_play_pause",void 0,e)}render(){if(!this.hass)return V``;const t=Ne(this.hass.states,this.players??[]);if(!t)return V`
    ...
    `],t([gt({attribute:!1})],ze.prototype,"players",void 0),t([bt()],ze.prototype,"_now",void 0),customElements.define("hub-now-playing",ze);const Fe=new Intl.NumberFormat("sv-SE");function Le(t,e){return e>0?Math.max(0,Math.min(100,t/e*100)):0}class je extends mt{_goto(){this.dispatchEvent(new CustomEvent("hub-goto-page",{detail:{page:"kcal"},bubbles:!0,composed:!0}))}render(){if(!this.hass)return V``;const t=this.todayEntity?this.getEntity(this.todayEntity):void 0,e=t?Number(t.state):NaN;if(!t||"unavailable"===t.state||"unknown"===t.state||Number.isNaN(e))return V`
        <div class="kc offline" @click=${this._goto}>
          <div class="ring" style="--pct:0"></div>
          <div class="meta"><b class="val">Kcal · offline</b></div>
        </div>
      `;const i="number"==typeof t.attributes.kcal_target?t.attributes.kcal_target:0,a=Le(e,i),s=function(t){const e=t.protein_g;return"number"==typeof e?`${Math.round(e)} g protein`:""}(t.attributes);return V`
      <div class="kc" @click=${this._goto}>
        <div class="ring" style="--pct:${a}"></div>
        <div class="meta">
          <b class="val">
            ${Fe.format(Math.round(e))}
            <span class="target">
              ${i>0?`/ ${Fe.format(i)} kcal`:"kcal"}
            </span>
          </b>
          ${s?V`<small class="sub">${s}</small>`:X}
        </div>
      </div>
    `}}je.styles=[Tt,o`
      :host {
    ...
    `],t([gt({attribute:!1})],je.prototype,"todayEntity",void 0),customElements.define("hub-kcal-ring",je);const De={frukost:"Frukost",lunch:"Lunch",middag:"Middag",mellis:"Mellis"},Be=["frukost","lunch","middag","mellis"],Oe={gymdag:"G",vilodag:"V",flexdag:"F"};const Ie=/^\d{4}-\d{2}-\d{2}$/;function Re(t){if(!t||"object"!=typeof t)return null;const e=t;if("frukost"!==(i=e.slot)&&"lunch"!==i&&"middag"!==i&&"mellis"!==i||"string"!=typeof e.name||""===e.name)return null;var i;const a=t=>"number"==typeof t&&Number.isFinite(t)?t:0;return{slot:e.slot,name:e.name,kcal:a(e.kcal),protein:a(e.protein),fat:a(e.fat),carbs:a(e.carbs),logged:!0===e.logged}}function He(t){if(!t||"object"!=typeof t)return null;const e=t;if("string"!=typeof e.date||!Ie.test(e.date))return null;const i=Array.isArray(e.meals)?e.meals.map(Re).filter(t=>null!==t):[];return{date:e.date,weekday:"string"==typeof e.weekday?e.weekday:"",day_type:"string"==typeof e.day_type?e.day_type:"vilodag",confirmed:!0===e.confirmed,meals:i,total_kcal:"number"==typeof e.total_kcal?e.total_kcal:0,target_kcal:"number"==typeof e.target_kcal?e.target_kcal:0,protein_ok:!0===e.protein_ok,kcal_ok:!1!==e.kcal_ok}}function Ue(t){if(!t)return null;const{week_start:e,today:i,days:a}=t;if("string"!=typeof e||!Ie.test(e))return null;if(!Array.isArray(a))return null;const s=a.map(He).filter(t=>null!==t);return 0===s.length?null:{weekStart:e,today:"string"==typeof i&&Ie.test(i)?i:e,confirmedDays:s.filter(t=>t.confirmed).length,days:s}}function Ve(t){const e=t=>Be.flatMap(e=>t.meals.filter(t=>t.slot===e&&!t.logged)),i=t.days.find(e=>e.date===t.today);if(i){const t=e(i);if(t.length>0)return{dayLabel:"Idag",day:i,meals:t}}const a=t.days.find(e=>e.date===function(t,e){const[i,a,s]=t.split("-").map(Number);return new Date(Date.UTC(i,a-1,s+e)).toISOString().slice(0,10)}(t.today,1));if(a){const t=e(a);if(t.length>0)return{dayLabel:"Imorgon",day:a,meals:t}}return null}const qe=new Intl.NumberFormat("sv-SE");class We extends mt{_model(){if(!this.plannerEntity)return null;const t=this.getEntity(this.plannerEntity);return t&&"unavailable"!==t.state&&"unknown"!==t.state?Ue(t.attributes):null}_open(){this.dispatchEvent(new CustomEvent("hub-goto-page",{detail:{page:"vecka"},bubbles:!0,composed:!0}))}render(){if(!this.hass||!this.plannerEntity)return X;const t=this._model(),e=t?Ve(t):null;return V`
      <div class="card${e?"":" empty"}" @click=${this._open}>
        <div class="head">
          <span class="eyebrow">Matsedel</span>
          ${t?V`<span class="count">${t.confirmedDays} / 7 ✓</span>`:X}
        </div>
        ${e?V`
              ...
              <div class="meals">
                ${e.meals.map(t=>V`
                    <div class="meal">
                      <span class="slot">${De[t.slot]}</span>
                      <span class="name">${t.name}</span>
                      <span class="kcal">${qe.format(t.kcal)}</span>
                    </div>
                  `)}
              </div>
            `:V`<div class="none">${t?"Inget planerat ännu":"Vecka · offline"}</div>`}
      </div>
    `}}function Xe(t){const e=Date.parse(t.expected??t.scheduled??"");return Number.isNaN(e)?Number.POSITIVE_INFINITY:e}function Ge(t,e,i){if(!Array.isArray(t))return[];const a=i?new RegExp(i,"i"):null;return t.filter(t=>t?.line?.designation===e).filter(t=>!(a&&t.destination&&a.test(t.destination))).sort((t,e)=>Xe(t)-Xe(e))}We.styles=[Tt,o`
      :host {
        display: block;
        height: 100%;
      }
    ...
    `],t([gt({attribute:!1})],We.prototype,"plannerEntity",void 0),customElements.define("hub-meal-card",We);function Ye(t){if(!Array.isArray(t))return[];const e=new Map;for(const i of t){if(!i||"object"!=typeof i)continue;const t=i;if("string"!=typeof t.header||0===t.header.length)continue;const a="number"==typeof t.priority?t.priority:0,s=Array.isArray(t.lines)?t.lines.map(t=>t&&"object"==typeof t?t.designation:null).filter(t=>"string"==typeof t&&t.length>0):[],r="string"==typeof t.details&&t.details.length>0?t.details:void 0,o="string"==typeof t.scope&&t.scope.length>0?t.scope:void 0,n=e.get(t.header);if(n){for(const t of s)n.badges.add(t);n.priority=Math.max(n.priority,a),!n.details&&r&&(n.details=r),!n.scope&&o&&(n.scope=o)}else e.set(t.header,{badges:new Set(s),priority:a,details:r,scope:o})}return[...e.entries()].sort((t,e)=>e[1].priority-t[1].priority).slice(0,5).map(([t,e])=>({badges:[...e.badges].sort(),header:t,...void 0!==e.details?{details:e.details}:{},...void 0!==e.scope?{scope:e.scope}:{}}))}const Ke=new Set(["unavailable","unknown",""]);function Ze(t){if(!t||Ke.has(t))return null;const e=new Date(t);if(Number.isNaN(e.getTime()))return null;return`${String(e.getHours()).padStart(2,"0")}:${String(e.getMinutes()).padStart(2,"0")}`}class Je extends mt{constructor(){super(...arguments),this._open=()=>{this.dispatchEvent(new CustomEvent("hub-transit-open",{bubbles:!0,composed:!0}))}}_pendeltag(){const t=this.config.transit?.pendeltag;if(!t)return V`<span class="sub dim">–</span>`;const e=this.getEntity(t.next_entity),i=Ze(e?.state);if(!i)return V`<span class="sub dim">–</span>`;const a=this.getEntity(t.count_entity),s=a&&!Number.isNaN(Number(a.state))?Number(a.state):null;return V`<span class="sub">Nästa ${i}${null===s?"":` · ${s} ${1===s?"avgång":"avgångar"}`}</span>`}_bus(){const t=this.config.transit?.bus;if(!t)return V`<span class="sub dim">Inga avgångar idag</span>`;const e=this.getEntity(t.entity),i=Ge(e?.attributes.departures??[],t.line,t.exclude_destination).slice(0,3);return 0===i.length?V`<span class="sub dim">Inga avgångar idag</span>`:V`<span class="sub"
      >${i.map((t,e)=>V`${e>0?V`<span class="sep">·</span>`:""}<span
            class="dep ${t.state&&"EXPECTED"!==t.state?"delayed":""}"
            >${t.display??"–"}</span
          >`)}</span
    >`}_shaped(){const t=this.config.disturbances_entity?this.getEntity(this.config.disturbances_entity):void 0;return t&&"unavailable"!==t.state&&"unknown"!==t.state?Ye(t.attributes.deviations):[]}_alerts(t){if(0===t.length)return X;if(1===t.length){const e=t[0];return V`<div class="alerts">
        <div class="alert">
    ...
    `],t([gt({attribute:!1})],Je.prototype,"config",void 0),customElements.define("hub-transit-card",Je);const Qe=36e5,ti=new Set(["unavailable","unknown","none",""]);function ei(t,e,i){if(!Array.isArray(t))return[];const a=[];for(const s of t){if(!s||"object"!=typeof s)continue;const t=s,r="number"==typeof t.total?t.total:Number(t.total);if(!Number.isFinite(r)||"string"!=typeof t.startsAt)continue;const o=new Date(t.startsAt);if(Number.isNaN(o.getTime()))continue;const n="number"==typeof t.energy?t.energy:Number(t.energy),l=Number.isFinite(n)?100*n:null,c=100*r,h="spot"===e&&null!==l?l:c+("allin"===e?i:0);a.push({start:o,ore:h,totalOre:c,spotOre:l})}return a.sort((t,e)=>t.start.getTime()-e.start.getTime())}function ii(t){if(t.length<3)return null;let e=1/0,i=-1;for(let a=0;a+3<=t.length;a++){let s=!0,r=t[a].ore;for(let e=a+1;e<a+3;e++){if(t[e].start.getTime()-t[e-1].start.getTime()!==Qe){s=!1;break}r+=t[e].ore}s&&r<e&&(e=r,i=a)}return i<0?null:{start:t[i].start,end:new Date(t[i+3-1].start.getTime()+Qe)}}function ai(t,e,i,a="allin",s=0){if(ti.has(String(e??"").toLowerCase()))return{now:null,level:"normal",today:[],tomorrow:[],cheapestWindow:null};const r=ei(t?.today,a,s),o=ei(t?.tomorrow,a,s);if(0===r.length&&0===o.length)return{now:null,level:"normal",today:[],tomorrow:[],cheapestWindow:null};const n=[...r,...o].sort((t,e)=>t.start.getTime()-e.start.getTime()),l=i.getTime(),c=n.find(t=>t.start.getTime()<=l&&l<t.start.getTime()+Qe)??null;let h="normal";if(c&&r.length){const t=r.reduce((t,e)=>t+e.ore,0)/r.length;if(t>0){const e=c.ore/t;e<.85?h="låg":e>1.15&&(h="hög")}}const d=n.filter(t=>t.start.getTime()+Qe>l);return{now:c,level:h,today:r,tomorrow:o,cheapestWindow:ii(d)}}function si(t){const e=[...t.today,...t.tomorrow];return e.length>0&&e.every(t=>null!==t.spotOre)}const ri="glass-hub-price-view";function oi(){try{return"spot"===localStorage.getItem(ri)?"spot":"allin"}catch{return"allin"}}function ni(t){return(t?.grid?.overforing_ore??0)+(t?.grid?.energiskatt_ore??0)}const li={"låg":"lågt",normal:"normalt","hög":"högt"};class ci extends mt{constructor(){super(...arguments),this._now=new Date,this._open=()=>{this.dispatchEvent(new CustomEvent("hub-goto-page",{detail:{page:"energi"},bubbles:!0,composed:!0}))}}connectedCallback(){super.connectedCallback(),this._interval=window.setInterval(()=>{this._now=new Date},6e4)}disconnectedCallback(){super.disconnectedCallback(),void 0!==this._interval&&(clearInterval(this._interval),this._interval=void 0)}_model(){const t=this.config.price_series_entity?this.getEntity(this.config.price_series_entity):void 0;if(!t)return null;const e=oi(),i=t.attributes,a=ai(i,t.state,this._now,e,"allin"===e?ni(this.config):0);return"spot"===e&&a.today.length&&a.today.some(t=>null===t.spotOre)?ai(i,t.state,this._now,"allin",ni(this.config)):a}_currentOre(t){if(t?.now)return Math.round(t.now.ore);const e=this.config.price_entity?this.getEntity(this.config.price_entity):void 0;return e&&!Number.isNaN(Number(e.state))?Math.round(100*Number(e.state)):null}_bars(t){const e=function(t,e){const i=[...t.today,...t.tomorrow].sort((t,e)=>t.start.getTime()-e.start.getTime()),a=e.getTime(),s=t.cheapestWindow,r=s?s.start.getTime():null,o=s?s.end.getTime():null;return i.filter(t=>t.start.getTime()+Qe>a).slice(0,12).map(t=>{const e=t.start.getTime();return{start:t.start,ore:t.ore,current:e<=a&&a<e+Qe,cheap:null!==r&&e>=r&&e<o}})}(t,this._now);if(0===e.length)return V`<div class="waiting">Väntar på prisdata</div>`;const i=e.map(t=>t.ore),a=Math.min(...i),s=Math.max(...i)-a;return V`<div class="bars">
      ${e.map(t=>{return V`<div
        ...
        >`:X;return V`
      <button class="card" aria-label=${null===e?"Elpris, öppna energisidan":`Elpris ${e} öre just nu${r?`, ${li[a]}`:""}, öppna energisidan`} @click=${this._open}>
        <div class="head">
          <div class="lead">
            <span class="ic">${Ot.bolt}</span>
            <span class="num ${s}">${null===e?"—":e}</span>
            <span class="unit">öre</span>
            ${r?V`<span class="level ${s}">· ${li[a]}</span>`:X}
          </div>
          ${n}
        </div>
    ...
    `],t([gt({attribute:!1})],di.prototype,"config",void 0),t([gt({attribute:!1})],di.prototype,"theme",void 0),t([gt({attribute:!1})],di.prototype,"weatherBg",void 0),t([gt({attribute:!1})],di.prototype,"pageActive",void 0),customElements.define("hub-home-page",di);const pi=new Set(["unavailable","unknown"]);class ui extends mt{constructor(){super(...arguments),this._flash=!1,this._longPressed=!1,this._downX=0,this._downY=0,this._onPointerDown=t=>{this._dead||(this._longPressed=!1,this._downX=t.clientX,this._downY=t.clientY,this._pressTimer=window.setTimeout(()=>{this._longPressed=!0,this.dispatchEvent(new CustomEvent("hub-light-open",{detail:{entity:this.light.entity,name:this.light.name},bubbles:!0,composed:!0}))},500))},this._onPointerMove=t=>{void 0!==this._pressTimer&&(Dt(t.clientX-this._downX)||Dt(t.clientY-this._downY))&&this._cancelPress()},this._cancelPress=()=>{void 0!==this._pressTimer&&(clearTimeout(this._pressTimer),this._pressTimer=void 0)},this._onClick=()=>{this._dead||(this._longPressed?this._longPressed=!1:(this.callService("light","toggle",void 0,this.light.entity),this._flash=!0,void 0!==this._flashTimer&&clearTimeout(this._flashTimer),this._flashTimer=window.setTimeout(()=>{this._flash=!1,this._flashTimer=void 0},200)))}}disconnectedCallback(){super.disconnectedCallback(),this._cancelPress(),void 0!==this._flashTimer&&(clearTimeout(this._flashTimer),this._flashTimer=void 0)}get _dead(){const t=this.getEntity(this.light.entity);return!t||pi.has(t.state)}get _stateLabel(){const t=this.getEntity(this.light.entity);if(!t||pi.has(t.state))return"Ej tillgänglig";if("on"!==t.state)return"Av";const e=t.attributes.brightness;return"number"==typeof e?`${Math.round(e/255*100)} %`:"På"}render(){if(!this.hass||!this.light)return V``;const t=this.isOn(this.light.entity),e=this._dead;return V`
      <div
    ...
    `}render(){if(!this.hass||!this.config)return V``;const t=this.config,e=function(t,e){let i=0,a=0;for(const s of t.rooms??[])for(const t of s.lights){const s=e[t.entity];bi(s)&&(a+=1,"on"===s.state&&(i+=1))}return{on:i,total:a}}(t,this.hass.states);return V`
      <div class="page">
        <div class="header">
          <div class="heading">
          ...
          <div class="actions">
            ${(t.scenes??[]).map(t=>V`
                ...
                >
                  ${Ot[t.icon]?V`<span class="ic">${Ot[t.icon]}</span>`:X}
                  <span>${t.name}</span>
                </button>
              `)}
            <button
        ...
        <div class="plot" style="grid-template-columns:${a}">
          ${t.map((a,s)=>{if("divider"===a.kind)return V`<div class="divider"></div>`;const r=this._height(a.hour.ore,e,i),o=a.cls.startsWith("future")?`background:${this._tint(a.hour.ore,e,i)}`:"";return V`
              ...
              >
                ...
                ${a.label&&this._detail!==s?V`<span class="cell-label">${a.label}</span>`:X}
    ...
    `],t([gt({attribute:!1})],mi.prototype,"model",void 0),t([gt({type:Number})],mi.prototype,"gridAddOre",void 0),t([bt()],mi.prototype,"_detail",void 0),customElements.define("hub-price-chart",mi);const fi={"låg":"lågt",normal:"normalt","hög":"högt"};class xi extends mt{constructor(){super(...arguments),this._now=new Date,this._view=oi()}connectedCallback(){super.connectedCallback(),this._interval=window.setInterval(()=>{this._now=new Date},6e4)}disconnectedCallback(){super.disconnectedCallback(),void 0!==this._interval&&(clearInterval(this._interval),this._interval=void 0)}_model(){const t=this.config.price_series_entity?this.getEntity(this.config.price_series_entity):void 0;if(!t)return null;const e=t.attributes;let i=ai(e,t.state,this._now,this._view,"allin"===this._view?ni(this.config):0);return"spot"!==this._view||si(i)||(i=ai(e,t.state,this._now,"allin",ni(this.config))),i}_setView(t){this._view=t,function(t){try{localStorage.setItem(ri,t)}catch{}}(t)}_currentOre(t){if(t?.now)return Math.round(t.now.ore);const e=this.config.price_entity?this.getEntity(this.config.price_entity):void 0;return e&&!Number.isNaN(Number(e.state))?Math.round(100*Number(e.state)):null}_chips(t){const e=this.config,i=[],a=e.co2_entity?this.getEntity(e.co2_entity):void 0;a&&!Number.isNaN(Number(a.state))&&i.push({icon:"leaf",label:`${Math.round(Number(a.state))} g CO₂`,tone:"green"});const s=e.fossil_entity?this.getEntity(e.fossil_entity):void 0;if(s&&!Number.isNaN(Number(s.state))){const t=Math.round(Number(s.state));i.push({icon:"leaf",label:`${t} % fossilt`,tone:t>=40?"coral":"green"})}const r=t?.cheapestWindow;if(r){const t=r.start.getHours(),e=r.end.getHours();i.push({icon:"clock",label:`Billigast ${t}–${e}`,tone:"green"})}return i}render(){if(!this.hass||!this.config)return V``;const t=this._model(),e=this._currentOre(t),i=t?.now?t.level:"normal",a=!!t&&t.today.length>0,s=this._chips(t),r=!!t&&si(t),o="låg"===i?"low":"hög"===i?"high":"",n=!!t?.now&&"normal"!==i;return V`
      <div class="page">
        <div class="header">
          <div class="head-row">
            <div>
        ...
        <div class="chart-wrap">
          ${a?V`<hub-price-chart .model=${t} .gridAddOre=${ni(this.config)}></hub-price-chart>`:V`<div class="waiting">Väntar på prisdata</div>`}
        </div>
    ...
    `}}async function yi(t){if(!t)return null;try{const i=await(e=t,new Promise((t,i)=>{const a=new Image;a.crossOrigin="anonymous",a.onload=()=>t(a),a.onerror=()=>i(new Error("image load failed")),a.src=e})),a=document.createElement("canvas");a.width=8,a.height=8;const s=a.getContext("2d");if(!s)return null;s.drawImage(i,0,0,8,8);const{data:r}=s.getImageData(0,0,8,8);let o=0,n=0,l=0,c=0;for(let t=0;t<r.length;t+=4){0!==r[t+3]&&(o+=r[t],n+=r[t+1],l+=r[t+2],c+=1)}return 0===c?null:[Math.round(o/c),Math.round(n/c),Math.round(l/c)]}catch{return null}var e}xi.styles=[Tt,o`
      :host {
        display: block;
        height: 100%;
      }
    ...
    `],t([gt({attribute:!1})],xi.prototype,"config",void 0),t([bt()],xi.prototype,"_now",void 0),t([bt()],xi.prototype,"_view",void 0),customElements.define("hub-energy-page",xi);const _i=new Set(["unavailable","unknown"]);class wi extends mt{constructor(){super(...arguments),this.groupMaster=null,this._drag=null}_entity(){return this.hass?.states[this.player.entity]}_volume(){if(null!==this._drag)return this._drag;const t=this._entity()?.attributes.volume_level;return"number"==typeof t?t:0}_onInput(t){this._drag=Number(t.target.value)}_onChange(t){const e=Number(t.target.value);this._drag=null,this.callService("media_player","volume_set",{volume_level:e},this.player.entity)}_stop(t){t.stopPropagation()}_toggleGroup(t){this.groupMaster&&(t?this.callService("media_player","unjoin",void 0,this.player.entity):this.callService("media_player","join",{group_members:[this.player.entity]},this.groupMaster))}render(){if(!this.hass||!this.player)return V``;const t=this._entity(),e=!t||_i.has(t.state),i=this._volume(),a=Math.round(100*i),s=!e&&i>0,r=this.player.entity===this.groupMaster,o=!r&&!!this.groupMaster&&(n=this.hass.states[this.groupMaster]?.attributes.group_members,l=this.player.entity,Array.isArray(n)&&n.includes(l));var n,l;const c=`linear-gradient(90deg, var(--hub-teal) 0 ${a}%, var(--hub-track) ${a}% 100%)`;return V`
      <div class="row ${s?"active":""}">
        <span class="ic">${Ot.speaker}</span>
        <div class="main">
          <div class="top">
            <span class="name">${this.player.name}</span>
            ${e?X:V`<span class="pct">${a}%</span>`}
          </div>
    ...
    `],t([gt({attribute:!1})],wi.prototype,"player",void 0),t([gt({attribute:!1})],wi.prototype,"groupMaster",void 0),t([bt()],wi.prototype,"_drag",void 0),customElements.define("hub-volume-row",wi);const ki=new Set(["off","unavailable","unknown","standby","idle"]);function $i(t){const e=Number.isFinite(t)&&t>0?t:0,i=Math.floor(e/60),a=Math.floor(e%60);return`${i}:${String(a).padStart(2,"0")}`}class Ei extends mt{constructor(){super(...arguments),this._sel=null,this._rgb=null,this._now=Date.now()}connectedCallback(){super.connectedCallback(),this._interval=window.setInterval(()=>{this._now=Date.now()},1e3)}disconnectedCallback(){super.disconnectedCallback(),void 0!==this._interval&&(clearInterval(this._interval),this._interval=void 0)}get _players(){return this.config?.media_players??[]}_selId(){if(this._sel)return this._sel;const t=Ne(this.hass?.states??{},this._players);return t?.entity.entity_id??this._players[0]?.entity??null}_theme(){const t=this.getRootNode()?.host;return"dag"===t?.getAttribute("data-theme")?"dag":"natt"}updated(t){const e=this._selId(),i=e?this.hass?.states[e]?.attributes.entity_picture:void 0;i!==this._pic&&(this._pic=i,i?yi(i).then(t=>{this._pic===i&&(this._rgb=t)}):this._rgb=null)}_transport(t,e){this.callService("media_player",t,void 0,e)}_hero(t,e){const i="playing"===t.state,a=t.attributes.media_title||e,s=t.attributes.media_artist||e,r=t.attributes.entity_picture,o="number"==typeof t.attributes.media_duration?t.attributes.media_duration:0,n=Pe(t,this._now),l=n/100*o,c=t.entity_id;return V`
      <div class="hero">
    ...
    `],t([gt({attribute:!1})],Ei.prototype,"config",void 0),t([bt()],Ei.prototype,"_sel",void 0),t([bt()],Ei.prototype,"_rgb",void 0),t([bt()],Ei.prototype,"_now",void 0),customElements.define("hub-media-page",Ei);let Ci=0;class Si extends ct{constructor(){super(...arguments),this.points=[],this.stroke="--hub-lavender",this.width=560,this.height=130,this._gid="hub-spark-"+Ci++}render(){const t=function(t,e,i,a=.1){const s=t.length;if(0===s)return[];if(1===s)return[{x:e,y:i/2}];const r=t.map(t=>t.value),o=Math.min(...r),n=Math.max(...r)-o,l=o-n*a,c=n*(1+2*a);return t.map((t,a)=>({x:a/(s-1)*e,y:n<=0?i/2:i-(t.value-l)/c*i}))}(this.points,this.width,this.height);if(0===t.length)return V``;const e=t.map(t=>`${t.x.toFixed(2)},${t.y.toFixed(2)}`).join(" "),i=t[t.length-1],a=t[0],s=t.length>=2,r=`${e} ${i.x.toFixed(2)},${this.height} ${a.x.toFixed(2)},${this.height}`;return V`
  ...
  `,t([gt({attribute:!1})],Si.prototype,"points",void 0),t([gt()],Si.prototype,"stroke",void 0),t([gt({type:Number})],Si.prototype,"width",void 0),t([gt({type:Number})],Si.prototype,"height",void 0),customElements.define("hub-sparkline",Si);const Mi=new Intl.NumberFormat("sv-SE"),Ai=new Intl.NumberFormat("sv-SE",{minimumFractionDigits:1,maximumFractionDigits:1}),Ti=new Intl.NumberFormat("sv-SE",{maximumFractionDigits:1}),Ni=new Intl.DateTimeFormat("sv-SE",{day:"numeric",month:"short",timeZone:"UTC"}),Pi=new Intl.DateTimeFormat("sv-SE",{weekday:"long",day:"numeric",month:"long",timeZone:"UTC"});function zi(t){if(!t)return"";const e=new Date(`${t}T00:00:00Z`);return Number.isNaN(e.getTime())?"":Ni.format(e).replace(/\.$/,"")}class Fi extends mt{_meals(t){const e=t.attributes.meals;return Array.isArray(e)?e.filter(t=>!!t&&"object"==typeof t).map(t=>({name:"string"==typeof t.name?t.name:"",kcal:"number"==typeof t.kcal?t.kcal:Number(t.kcal)||0})).filter(t=>t.name):[]}_num(t){return"number"==typeof t?t:NaN}_offline(){return V`
      <div class="page">
        <div class="offline">
    ...
    `}_weightCard(){const t=this.config.kcal?.forecast_entity,e=t?this.getEntity(t):void 0,i=e?Number(e.state):NaN;if(!e||"unavailable"===e.state||"unknown"===e.state||Number.isNaN(i))return V`
      ...
      `;const a=e.attributes.weight_trend,s=Array.isArray(a)?a.filter(t=>!!t&&"object"==typeof t).map(t=>({date:String(t.date??""),value:Number(t.kg)})).filter(t=>Number.isFinite(t.value)):[],r=function(t){if(t.length<2)return null;const e=new Date(`${t[0].date}T00:00:00Z`).getTime(),i=new Date(`${t[t.length-1].date}T00:00:00Z`).getTime();return Number.isNaN(e)||Number.isNaN(i)?null:Math.round((i-e)/864e5)}(s),o=s.length>=2?s[s.length-1].value-s[0].value:null,n=null===o||null===r?null:`${o<0?"−":o>0?"+":""}${Ai.format(Math.abs(o))} kg på ${r} ${1===r?"dag":"dagar"}`,l=e.attributes.forecast,c=l&&"object"==typeof l?l:null,h=c?function(t){const e="number"==typeof t.goal_kg?`Mål ${Ti.format(t.goal_kg)} kg`:"",i=t.eta?zi(t.eta):"",a=t.eta_early&&t.eta_late?`${zi(t.eta_early)}–${zi(t.eta_late)}`:"";return[e,i?`ETA ${i}${a?` (${a})`:""}`:""].filter(Boolean).join(" · ")}(c):"",d=!!c?.on_track;return V`
      <section class="card">
        <span class="w-eyebrow">Vikt</span>
        <div class="w-num-row">
          <span class="w-num">${Ai.format(i)}</span>
          <span class="w-unit">kg</span>
        </div>
        ${n?V`<span class="w-delta">${n}</span>`:X}
        ...
        <div class="forecast">
          ...
          ${d?V`<span class="fc-chip">i fas ✓</span>`:X}
        </div>
      </section>
    `}render(){if(!this.hass||!this.config)return V``;const t=this.config.kcal?.today_entity,e=t?this.getEntity(t):void 0,i=e?Number(e.state):NaN;if(!e||"unavailable"===e.state||"unknown"===e.state||Number.isNaN(i))return this._offline();const a=this._num(e.attributes.kcal_target),s=Le(i,a),r=Number.isFinite(a)&&a>0,o=r?a-i:NaN,n=r?o>0?`${Mi.format(Math.round(o))} kcal kvar`:0===o?"Målet nått":`${Mi.format(Math.round(-o))} över målet`:null,l=this._num(e.attributes.protein_g),c=this._num(e.attributes.protein_target_g),h=Number.isFinite(l)&&Number.isFinite(c)&&c>0,d=h?Math.max(0,Math.min(100,l/c*100)):0,p=this._meals(e),u=e.attributes.date,g="string"!=typeof u||Number.isNaN(new Date(`${u}T00:00:00Z`).getTime())?"":Pi.format(new Date(`${u}T00:00:00Z`));return V`
      <div class="page">
        <div class="header">
          <h1 class="title">Kcal</h1>
          ${g?V`<span class="subtitle">${g}</span>`:X}
        </div>
          ...
          <section class="card">
            <div class="ring-wrap">
              ...
              <div class="ring-center">
                <span class="kc-num">${Mi.format(Math.round(i))}</span>
                <span class="kc-target">
                  ${r?`/ ${Mi.format(a)} kcal`:"kcal"}
                </span>
              </div>
            </div>
            ${n?V`<div class="kc-remain">${n}</div>`:X}
            ...
            <div class="meals">
              ...
              ${p.length?p.map(t=>V`
                      <div class="meal">
                        ...
                        <span class="meal-kcal">${Mi.format(Math.round(t.kcal))} kcal</span>
                      </div>
    ...
    `],t([gt({attribute:!1})],Fi.prototype,"config",void 0),customElements.define("hub-kcal-page",Fi);const Li=new Intl.NumberFormat("sv-SE"),ji=new Intl.DateTimeFormat("sv-SE",{weekday:"long",day:"numeric",month:"long",timeZone:"UTC"});class Di extends mt{constructor(){super(...arguments),this._openDate=null,this._confirming=!1}_model(){const t=this.config?.kcal?.planner_entity;if(!t)return null;const e=this.getEntity(t);return e&&"unavailable"!==e.state&&"unknown"!==e.state?Ue(e.attributes):null}_confirm(t){this._confirming||t.confirmed||0===t.meals.length||(this._confirming=!0,this.callService("rest_command","kcal_confirm_day",{date:t.date}),window.setTimeout(()=>{const t=this.config?.kcal?.planner_entity;t&&this.callService("homeassistant","update_entity",void 0,t),this._confirming=!1,this._openDate=null},1500))}_dayPopup(t){const e=t.days.find(t=>t.date===this._openDate);if(!e)return X;const i=ji.format(new Date(`${e.date}T00:00:00Z`)),a=!e.confirmed&&e.meals.some(t=>!t.logged);return V`
      <div class="scrim" @click=${()=>this._openDate=null}>
        <div class="popup" @click=${t=>t.stopPropagation()}>
          <h2 class="popup-title">${i}</h2>
          <div class="popup-sub">
            ${e.day_type} · ${Li.format(e.total_kcal)} / ${Li.format(e.target_kcal)} kcal
            ${e.confirmed?" · bekräftad ✓":""}
          </div>
          ${e.meals.map(t=>V`
              <div class="pm">
                <div>
                  <span class="pm-slot">${De[t.slot]}${t.logged?" · loggad":""}</span>
                  <span class="pm-name">${t.name}</span>
                </div>
                <span class="pm-macro">
                  ${Li.format(t.kcal)} kcal<br />
                  P ${Li.format(t.protein)} · F ${Li.format(t.fat)} · K ${Li.format(t.carbs)}
                </span>
              </div>
            `)}
          ${0===e.meals.length?V`<div class="empty-day">Inget planerat.</div>`:X}
          <div class="popup-actions">
            ${e.confirmed?V`<span class="confirmed-note">Dagen är låst ✓</span>`:X}
    ...
    `}_dayCard(t,e){const i=Be.filter(e=>t.meals.some(t=>t.slot===e));return V`
      <button
      ...
      >
        <div class="day-head">
          ...
          ${t.confirmed?V`<span class="lock">✓</span>`:X}
          <span class="type-chip ${t.day_type}">${a=t.day_type,Oe[a]??"·"}</span>
        </div>
        <div class="slots">
          ${0===i.length?V`<span class="empty-day">—</span>`:X}
          ${i.map(e=>V`
              <div>
                <span class="slot-label">${De[e]}</span>
                ${t.meals.filter(t=>t.slot===e).map(t=>V`
                      <div class="meal">
                        <span class="meal-name${t.logged?" logged":""}">${t.name}</span>
                        <span class="meal-kcal">${Li.format(t.kcal)} kcal</span>
                      </div>
                    `)}
              </div>
            `)}
        ...
        <div class="day-foot">
          ${t.meals.length>0?V`${Li.format(t.total_kcal)} / ${Li.format(t.target_kcal)}
              ${t.kcal_ok&&t.protein_ok?X:V`<span class="warn"> ⚠</span>`}`:V`&nbsp;`}
        </div>
      </button>
    ...
    `:V`<div class="page"><div class="offline">Vecka · offline</div></div>`}}Di.styles=[Tt,o`
      :host {
        display: block;
        height: 100%;
    ...
    `],t([gt({attribute:!1})],Di.prototype,"config",void 0),t([bt()],Di.prototype,"_openDate",void 0),t([bt()],Di.prototype,"_confirming",void 0),customElements.define("hub-planner-page",Di);const Bi=q`
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
    <path d="M6 6l12 12M18 6L6 18"></path>
  </svg>
`;class Oi extends mt{constructor(){super(...arguments),this.room=null,this._onScrim=t=>{t.target===t.currentTarget&&this._close()}}_close(){this.dispatchEvent(new CustomEvent("hub-popup-close",{bubbles:!0,composed:!0}))}_activateScene(t){this.callService("scene","turn_on",void 0,t)}render(){if(!this.room||!this.hass)return V``;const t=this.room;return V`
    ...
    `],t([gt({attribute:!1})],Oi.prototype,"room",void 0),customElements.define("hub-room-popup",Oi);const Ii=q`
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
    <path d="M6 6l12 12M18 6L6 18"></path>
  </svg>
`;class Ri extends mt{constructor(){super(...arguments),this.entity="",this.name="",this._onScrim=t=>{t.target===t.currentTarget&&this._close()}}_close(){this.dispatchEvent(new CustomEvent("hub-popup-close",{bubbles:!0,composed:!0}))}render(){return this.entity&&this.hass?V`
      <div class="scrim" @click=${this._onScrim}>
...
`,Ui=new Set(["EXPECTED","ATSTOP"]);class Vi extends mt{constructor(){super(...arguments),this._onScrim=t=>{t.target===t.currentTarget&&this._close()}}_close(){this.dispatchEvent(new CustomEvent("hub-popup-close",{bubbles:!0,composed:!0}))}_departures(t,e,i){if(!t)return[];const a=this.getEntity(t);return Ge(a?.attributes.departures??[],e,i).slice(0,6)}_depRow(t){const e=function(t){return"string"==typeof t.state&&t.state.length>0&&!Ui.has(t.state)}(t),i=Ze(t.expected??t.scheduled)??"–";return V`
    ...
    `}_storSection(t){return 0===t.length?X:V`
      <div class="section">
        ...
        ${t.map(t=>V`
            <div class="stor">
              ...
              ${t.details?V`<div class="stor-details">${t.details}</div>`:X}
              ${t.scope?V`<div class="stor-scope">Berör: ${t.scope}</div>`:X}
            </div>
          `)}
      </div>
    `}render(){if(!this.hass||!this.config)return V``;const t=this.config,e=this._departures(t.departures?.list_entity,"43",""),i=t.transit?.bus,a=i?this._departures(i.entity,i.line,i.exclude_destination):[],s=t.disturbances_entity?this.getEntity(t.disturbances_entity):void 0,r=s&&"unavailable"!==s.state&&"unknown"!==s.state?Ye(s.attributes.deviations):[];return V`
      <div class="scrim" @click=${this._onScrim}>
        <div class="card" role="dialog" aria-label="Resor och störningar">
          <div class="head">
            <span class="title">Resor & störningar</span>
            <button class="close" aria-label="Stäng" @click=${this._close}>${Hi}</button>
          </div>
    ...
    `],t([gt({attribute:!1})],Vi.prototype,"config",void 0),customElements.define("hub-transit-popup",Vi);const qi=q`
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
    <path d="M6 6l12 12M18 6L6 18"></path>
  </svg>
`,Wi=new Intl.DateTimeFormat("sv-SE",{weekday:"short"});class Xi extends mt{constructor(){super(...arguments),this._loc=0,this._hours=[],this._days=[],this._bgOn=zt(),this._loadedFor="",this._onScrim=t=>{t.target===t.currentTarget&&this._close()}}get _locations(){const t=this.config;return t?.weather_locations?.length?t.weather_locations:t?.weather_entity?[{entity:t.weather_entity,name:"Hem"}]:[]}updated(t){const e=this._locations[this._loc]?.entity;e&&this.hass&&(t.has("hass")||t.has("config"))&&this._loadedFor!==e&&this._load(e)}async _load(t){this._loadedFor=t;const[e,i]=await Promise.all([xe(this.hass,t,"hourly"),xe(this.hass,t,"daily")]);this._loadedFor===t&&(this._hours=e?Wt(e):[],this._days=i?Xt(i):[])}_pickLoc(t){if(t===this._loc)return;this._loc=t,this._hours=[],this._days=[];const e=this._locations[t]?.entity;e&&this._load(e)}_close(){this.dispatchEvent(new CustomEvent("hub-popup-close",{bubbles:!0,composed:!0}))}_toggleBg(){this._bgOn=!this._bgOn,Ft(this._bgOn),this.dispatchEvent(new CustomEvent("hub-weather-bg-toggle",{detail:{on:this._bgOn},bubbles:!0,composed:!0}))}get _isNight(){return"below_horizon"===this.hass?.states["sun.sun"]?.state}_hero(){const t=this.getEntity(this._locations[this._loc]?.entity??"");if(!t)return X;const e=t.attributes.temperature,i=t.attributes.apparent_temperature,a=t.attributes.wind_speed,s=t.attributes.wind_speed_unit??"km/h";return V`
      <div class="hero">
        ...
        <span class="hero-meta">
          ...
          ${"number"==typeof i?V`<span>Känns som ${Math.round(i)}°</span>`:X}
          ${"number"==typeof a?V`<span>Vind ${Math.round(a)} ${s}</span>`:X}
        </span>
      </div>
    ...
    `}_daily(){const t=this._days.slice(0,7),e=function(t){if(0===t.length)return null;let e=1/0,i=-1/0;for(const a of t)null!==a.low&&a.low<e&&(e=a.low),a.high>i&&(i=a.high),a.high<e&&(e=Math.min(e,a.high));return Number.isFinite(e)&&Number.isFinite(i)?{min:e,max:i}:null}(t),i=e?Math.max(e.max-e.min,1):1;return V`
      <div class="section">
        ...
        ${t.length&&e?t.map((t,a)=>{const s=t.low??t.high,r=(s-e.min)/i*100,o=Math.max((t.high-s)/i*100,4);return V`
                <div class="day-row">
                  <span class="day-name">${0===a?"Idag":function(t){return t.length?t.charAt(0).toUpperCase()+t.slice(1):t}(Wi.format(new Date(t.ts)))}</span>
                  ${me(t.condition,!1)}
      ...
      <div class="scrim" @click=${this._onScrim}>
        <div class="card" role="dialog" aria-label="Väder">
          <div class="head">
            ...
            <button class="close" aria-label="Stäng" @click=${()=>this._close()}>${qi}</button>
          </div>
          ${this._hero()}
          ${this._hourly()}
          ${this._daily()}
    ...
    `],t([gt({attribute:!1})],Xi.prototype,"config",void 0),t([bt()],Xi.prototype,"_loc",void 0),t([bt()],Xi.prototype,"_hours",void 0),t([bt()],Xi.prototype,"_days",void 0),t([bt()],Xi.prototype,"_bgOn",void 0),customElements.define("hub-weather-popup",Xi);const Gi=o`
  :host {
    position: absolute;
    inset: 0;
    z-index: 40;
...
`;class Yi extends mt{constructor(){super(...arguments),this._onScrim=t=>{t.target===t.currentTarget&&this._close()},this._allOff=()=>{const t=[...new Set((this.config.rooms??[]).flatMap(t=>t.lights.map(t=>t.entity)))];this.hass?.callService("light","turn_off",{},{entity_id:t})}}_close(){this.dispatchEvent(new CustomEvent("hub-popup-close",{bubbles:!0,composed:!0}))}_scene(t){this.callService("scene","turn_on",void 0,t)}render(){if(!this.hass||!this.config)return V``;const t=this.config.rooms??[];return V`
    ...
    `],t([gt({attribute:!1})],Yi.prototype,"config",void 0),customElements.define("hub-lights-modal",Yi);class Ki extends mt{constructor(){super(...arguments),this._onScrim=t=>{t.target===t.currentTarget&&this._close()}}_close(){this.dispatchEvent(new CustomEvent("hub-popup-close",{bubbles:!0,composed:!0}))}_press(t){this.callService("button","press",void 0,t)}_vac(t){this.config.vacuum_entity&&this.callService("vacuum",t,void 0,this.config.vacuum_entity)}_selectOption(t,e){this.callService("select","select_option",{option:e},t)}_selectChips(t){if(!t)return X;const e=this.getEntity(t),i=e?.attributes.options??[];return i.length?V`<div class="chips">
      ${i.map(i=>V`
          <button class="chip ${e?.state===i?"sel":""}" @click=${()=>this._selectOption(t,i)}>
            ${i}
          </button>
        `)}
    </div>`:X}render(){if(!this.hass||!this.config)return V``;const t=this.config.vacuum_controls,e=this.config.vacuum_entity?this.getEntity(this.config.vacuum_entity):void 0,i=e?.state??"unknown",a=t?.status_entity?this.getEntity(t.status_entity)?.state:i,s=t?.battery_entity?this.getEntity(t.battery_entity)?.state:void 0,r=t?.current_room_entity?this.getEntity(t.current_room_entity)?.state:void 0,o="cleaning"===i||"returning"===i,n="paused"===i;return V`
      <div class="scrim" @click=${this._onScrim}>
        <div class="card" role="dialog" aria-label="Roborock">
          ...
          ${t?.mop_mode_entity?V`<div class="sect">Mopläge</div>${this._selectChips(t.mop_mode_entity)}`:X}
          ${t?.mop_intensity_entity?V`<div class="sect">Moppintensitet</div>${this._selectChips(t.mop_intensity_entity)}`:X}
    ...
    `],t([gt({attribute:!1})],Ki.prototype,"config",void 0),customElements.define("hub-vacuum-popup",Ki);const Zi={charging:"Laddar",discharging:"Urladdar",done:"Färdigladdad",idle:"Vilar",scheduled:"Schemalagd",not_charging:"Laddar inte",error:"Fel",fault:"Fel"};class Ji extends mt{constructor(){super(...arguments),this._onScrim=t=>{t.target===t.currentTarget&&this._close()}}_close(){this.dispatchEvent(new CustomEvent("hub-popup-close",{bubbles:!0,composed:!0}))}_climate(){const t=this.config.volvo?.climate_entity;t&&(t.startsWith("switch.")?this.callService("switch","toggle",void 0,t):this.callService("button","press",void 0,t))}_climateStop(){const t=this.config.volvo?.climate_stop_entity;t&&this.callService("button","press",void 0,t)}_lockAction(t){const e=this.config.volvo?.lock_entity;e&&this.callService("lock",t,void 0,e)}_val(t,e=""){if(!t)return"–";const i=this.getEntity(t);if(!i||"unavailable"===i.state||"unknown"===i.state)return"–";const a=e||(i.attributes.unit_of_measurement??"");return a?`${i.state} ${a}`:i.state}_chargingLabel(t){if(!t)return"–";const e=this.getEntity(t);return e&&"unavailable"!==e.state&&"unknown"!==e.state?Zi[e.state]??e.state.replace(/_/g," "):"–"}render(){if(!this.hass||!this.config?.volvo)return V``;const t=this.config.volvo,e=t.climate_entity?.startsWith("switch.")??!1,i=e&&t.climate_entity?this.getEntity(t.climate_entity):void 0,a=e&&"on"===i?.state,s=t.lock_entity?this.getEntity(t.lock_entity)?.state:void 0;return V`
      <div class="scrim" @click=${this._onScrim}>
        <div class="card" role="dialog" aria-label=${t.name??"Volvo"}>
          ...
          <div class="actions">
            ${t.climate_entity?V`<button class="act primary ${a?"on":""}" @click=${()=>this._climate()}>
                  ${a?"Klimat på — stäng av":"Starta klimat"}
                </button>`:X}
            ${t.climate_stop_entity&&!t.climate_entity?.startsWith("switch.")?V`<button class="act" style="grid-column: span 2" @click=${()=>this._climateStop()}>
                  Stoppa klimat
                </button>`:X}
            ${t.lock_entity?V`
          ...
          <div class="grid">
            ...
            ${(t.doors??[]).map(t=>{const e="on"===this.getEntity(t.entity)?.state;return V`<div class="row">
    ...
    `],t([gt({attribute:!1})],Ji.prototype,"config",void 0),customElements.define("hub-car-popup",Ji);class Qi extends mt{constructor(){super(...arguments),this._items=null,this._lastCount="",this._fetchSeq=0,this._onScrim=t=>{t.target===t.currentTarget&&this._close()}}updated(t){super.updated(t);const e=this.config?.todo_entity;if(!e||!this.hass)return;const i=this.getEntity(e)?.state??"";i!==this._lastCount&&(this._lastCount=i,this._refresh())}async _refresh(){if(!this.hass||!this.config?.todo_entity)return;const t=++this._fetchSeq,e=await Me(this.hass,this.config.todo_entity);t===this._fetchSeq&&(this._items=e)}_close(){this.dispatchEvent(new CustomEvent("hub-popup-close",{bubbles:!0,composed:!0}))}async _add(){const t=this.shadowRoot?.querySelector("input"),e=t?.value.trim();e&&this.config.todo_entity&&(this.callService("todo","add_item",{item:e},this.config.todo_entity),t&&(t.value=""))}_toggle(t){if(!this.config.todo_entity)return;const e="completed"===t.status?"needs_action":"completed";this.callService("todo","update_item",{item:t.uid,status:e},this.config.todo_entity),window.setTimeout(()=>{this._refresh()},400)}_clearDone(){this.config.todo_entity&&(this.callService("todo","remove_completed_items",void 0,this.config.todo_entity),window.setTimeout(()=>{this._refresh()},400))}render(){if(!this.hass||!this.config?.todo_entity)return V``;const{open:t,done:e}=Se(this._items),i=V`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"><path d="M5 13l4 4L19 7"></path></svg>`;return V`
      <div class="scrim" @click=${this._onScrim}>
        <div class="card" role="dialog" aria-label="Att göra">
          ...
          ${[...t,...e].map(t=>V`
            ...
            `)}
          ${e.length?V`<button class="clear" @click=${()=>this._clearDone()}>Rensa klara (${e.length})</button>`:X}
        </div>
      </div>
    `}}Qi.styles=[Tt,Gi,o`
      .add {
    ...
    `],t([gt({attribute:!1})],Qi.prototype,"config",void 0),t([bt()],Qi.prototype,"_items",void 0),customElements.define("hub-todo-popup",Qi);const ta={hem:{label:"Hem",icon:"home",tone:"neutral"},ljus:{label:"Ljus",icon:"lamp",tone:"amber"},media:{label:"Media",icon:"note",tone:"teal"},energi:{label:"Energi",icon:"bolt",tone:"green"},kcal:{label:"Kcal",icon:"ring",tone:"lavender"},vecka:{label:"Vecka",icon:"calendar",tone:"lavender"}};class ea extends ct{constructor(){super(...arguments),this.pages=[],this.active=0}_select(t){this.dispatchEvent(new CustomEvent("hub-goto-page",{detail:{page:t},bubbles:!0,composed:!0}))}render(){return V`
      <nav>
        <div class="rail"></div>
        <div class="items">
          ${this.pages.map((t,e)=>{const i=function(t){const e=ta[t];return e?{id:t,...e}:{id:t,label:t.charAt(0).toUpperCase()+t.slice(1),icon:"",tone:"neutral"}}(t),a=e===this.active,s=Ot[i.icon];return V`
              <button
    ...
    `],t([gt({attribute:!1})],ea.prototype,"pages",void 0),t([gt({type:Number})],ea.prototype,"active",void 0),customElements.define("hub-nav-bar",ea);const ia=["hem","ljus","media","energi","kcal","vecka"],aa={hem:"Hem",ljus:"Ljus",media:"Media",energi:"Energi",kcal:"Kcal",vecka:"Vecka"};const sa=["auto","dag","natt"];let ra=0;class oa extends mt{constructor(){super(...arguments),this.theme="natt",this.kiosk=new URLSearchParams(location.search).has("kiosk"),this._page=0,this._dragX=0,this._openRoom=null,this._openLight=null,this._openTransit=!1,this._openWeather=!1,this._openLights=!1,this._openVacuum=!1,this._openCar=!1,this._openTodo=!1,this._weatherBgOn=zt(),this._override=function(){const t=localStorage.getItem(Nt);return"natt"===t||"dag"===t?t:"auto"}(),this._pointerActive=!1,this._dragging=!1,this._startX=0,this._startY=0,this._lastX=0,this._lastT=0,this._velocity=0,this._onRoomOpen=t=>{const e=t.detail?.roomId;this._openRoom=this._cfg?.rooms?.find(t=>t.id===e)??null},this._onLightOpen=t=>{const e=t.detail;this._openLight=e?.entity?{entity:e.entity,name:e.name??e.entity}:null},this._onGotoPage=t=>{const e=t.detail?.page;e&&this.goToPage(e)},this._onTransitOpen=()=>{this._openTransit=!0},this._onLightsOpen=()=>{this._openLights=!0},this._onVacuumOpen=()=>{this._openVacuum=!0},this._onCarOpen=()=>{this._openCar=!0},this._onTodoOpen=()=>{this._openTodo=!0},this._onWeatherOpen=()=>{this._openWeather=!0},this._onWeatherBgToggle=t=>{this._weatherBgOn=t.detail?.on??zt(),Ft(this._weatherBgOn)},this._onPopupClose=()=>{this._openRoom=null,this._openLight=null,this._openTransit=!1,this._openWeather=!1,this._openLights=!1,this._openVacuum=!1,this._openCar=!1,this._openTodo=!1},this._onAnyInteraction=()=>{this._resetIdle()},this._onPointerDown=t=>{this._pointerActive=!0,this._dragging=!1,this._startX=t.clientX,this._startY=t.clientY,this._lastX=t.clientX,this._lastT=t.timeStamp,this._velocity=0,this._dragX=0},this._onPointerMove=t=>{if(!this._pointerActive)return;const e=t.clientX-this._startX,i=t.clientY-this._startY;if(!this._dragging){if(!Dt(e)&&!Dt(i))return;if(!function(t,e){return Math.abs(t)>Math.abs(e)}(e,i))return void(this._pointerActive=!1);this._dragging=!0,t.currentTarget.setPointerCapture?.(t.pointerId),this._lastX=t.clientX,this._lastT=t.timeStamp}const a=t.timeStamp-this._lastT;a>0&&(this._velocity=(t.clientX-this._lastX)/a),this._lastX=t.clientX,this._lastT=t.timeStamp,this._dragX=e},this._onPointerUp=t=>{if(!this._pointerActive)return;const e=this._dragging;if(this._pointerActive=!1,this._dragging=!1,e){t.currentTarget.releasePointerCapture?.(t.pointerId);const e=this.clientWidth||window.innerWidth;this._page=function(t,e,i,a,s){const r=.2*e,o=Math.abs(i)>.5;let n=a;return t<-r||o&&i<-.5?n=a+1:(t>r||o&&i>.5)&&(n=a-1),Math.max(0,Math.min(s-1,n))}(this._dragX,e,this._velocity,this._page,this._pages.length),ra=this._page}this._dragX=0,this._velocity=0}}setConfig(t){super.setConfig(t)}get _cfg(){return this._config}get _pages(){return this._cfg?.pages??ia}connectedCallback(){super.connectedCallback(),function(){if(document.getElementById("glass-hub-fonts"))return;const t=document.createElement("style");t.id="glass-hub-fonts",t.textContent="\n@font-face{font-family:'Outfit';src:url('/local/glass-cards/fonts/outfit-variable.woff2') format('woff2-variations');font-weight:100 900;font-display:swap;}\n@font-face{font-family:'Inter';src:url('/local/glass-cards/fonts/inter-variable.woff2') format('woff2-variations');font-weight:100 900;font-display:swap;}\n",document.head.appendChild(t)}(),this._applyTheme(),this._page=ra,this._resetIdle(),this._startKioskDrawerShim(),function(){const t=new URLSearchParams(location.search).get("weather");t&&(Lt=t),window.__hubForceWeather=t=>{Lt=t,window.dispatchEvent(new CustomEvent("hub-weather-force"))}}(),this.addEventListener("pointerdown",this._onAnyInteraction),this.addEventListener("hub-room-open",this._onRoomOpen),this.addEventListener("hub-light-open",this._onLightOpen),this.addEventListener("hub-transit-open",this._onTransitOpen),this.addEventListener("hub-goto-page",this._onGotoPage),this.addEventListener("hub-popup-close",this._onPopupClose),this.addEventListener("hub-weather-open",this._onWeatherOpen),this.addEventListener("hub-lights-open",this._onLightsOpen),this.addEventListener("hub-vacuum-open",this._onVacuumOpen),this.addEventListener("hub-car-open",this._onCarOpen),this.addEventListener("hub-todo-open",this._onTodoOpen),this.addEventListener("hub-weather-bg-toggle",this._onWeatherBgToggle)}disconnectedCallback(){super.disconnectedCallback(),this._clearIdle(),void 0!==this._kioskTimer&&(clearInterval(this._kioskTimer),this._kioskTimer=void 0),this.removeEventListener("pointerdown",this._onAnyInteraction),this.removeEventListener("hub-room-open",this._onRoomOpen),this.removeEventListener("hub-light-open",this._onLightOpen),this.removeEventListener("hub-transit-open",this._onTransitOpen),this.removeEventListener("hub-goto-page",this._onGotoPage),this.removeEventListener("hub-popup-close",this._onPopupClose),this.removeEventListener("hub-weather-open",this._onWeatherOpen),this.removeEventListener("hub-lights-open",this._onLightsOpen),this.removeEventListener("hub-vacuum-open",this._onVacuumOpen),this.removeEventListener("hub-car-open",this._onCarOpen),this.removeEventListener("hub-todo-open",this._onTodoOpen),this.removeEventListener("hub-weather-bg-toggle",this._onWeatherBgToggle)}willUpdate(t){t.has("hass")&&this._applyTheme()}goToPage(t){const e=this._pages.indexOf(t);e>=0&&(this._page=e,ra=e,this._dragX=0)}_applyTheme(){const t=this.hass?.states["sun.sun"]?.attributes?.elevation,e="number"==typeof t?t:null;this.theme=function(t,e,i=4){return"auto"!==e?e:null===t?"natt":t>i?"dag":"natt"}(e,this._override,this._cfg?.day_elevation??4)}_cycleTheme(){const t=sa.indexOf(this._override);this._override=sa[(t+1)%sa.length],function(t){localStorage.setItem(Nt,t)}(this._override),this._applyTheme()}_toggleKiosk(){const t=new URLSearchParams(location.search);t.has("kiosk")?t.delete("kiosk"):t.set("kiosk","true");const e=t.toString();location.assign(location.pathname+(e?`?${e}`:""))}_resetIdle(){this._clearIdle();const t=this._cfg?.idle_return_s??120;this._idleTimer=window.setTimeout(()=>{0!==this._page&&this.goToPage(this._pages[0])},1e3*t)}_clearIdle(){void 0!==this._idleTimer&&(clearTimeout(this._idleTimer),this._idleTimer=void 0)}_startKioskDrawerShim(){if(!new URLSearchParams(location.search).has("kiosk"))return;const t=Date.now(),e=()=>{const t=document.querySelector("home-assistant")?.shadowRoot?.querySelector("home-assistant-main");if(!t)return!1;t.style.setProperty("--mdc-drawer-width","0px");const e=t.shadowRoot?.querySelector("ha-drawer");return e?.style.setProperty("--mdc-drawer-width","0px"),!0};e()||(this._kioskTimer=window.setInterval(()=>{(e()||Date.now()-t>5e3)&&(clearInterval(this._kioskTimer),this._kioskTimer=void 0)},250))}_themeGlyph(){return"auto"===this._override?V`<span class="glyph-auto">A</span>`:"dag"===this._override?q`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"
                      ...
                      ></hub-energy-page>`:"media"===e?V`<hub-media-page
                        ...
                        ></hub-media-page>`:"kcal"===e?V`<hub-kcal-page
                          ...
                          ></hub-kcal-page>`:"vecka"===e?V`<hub-planner-page
                              .hass=${this.hass}
                              .config=${this._cfg}
                            ></hub-planner-page>`:V`<h1 class="page-placeholder">${function(t){return aa[t]??t.charAt(0).toUpperCase()+t.slice(1)}(e)}</h1>`}
            </section>
          `)}
      </div>
      ...
      ${this._openWeather?V`<hub-weather-popup
            ...
            .config=${this._cfg}
          ></hub-weather-popup>`:X}
      ${this._openLights?V`<hub-lights-modal .hass=${this.hass} .config=${this._cfg}></hub-lights-modal>`:X}
      ${this._openVacuum?V`<hub-vacuum-popup .hass=${this.hass} .config=${this._cfg}></hub-vacuum-popup>`:X}
      ${this._openCar?V`<hub-car-popup .hass=${this.hass} .config=${this._cfg}></hub-car-popup>`:X}
      ${this._openTodo?V`<hub-todo-popup .hass=${this.hass} .config=${this._cfg}></hub-todo-popup>`:X}
    `}}oa.styles=[Tt,o`
      :host {
        position: absolute;
        inset: 0;
