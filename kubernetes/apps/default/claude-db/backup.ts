...
const srcPath = process.env.SRC ?? "/data/claude.db";
...
const weekday = new Date()
  ...
  .toLowerCase();
const dest = `/backups/claude-${weekday}.db`;
await Bun.write(dest, snapshot);

// Restore check: the snapshot must open and contain the fragrances table.
const check = new Database(dest, { readonly: true });
const row = check.query<{ n: number }, []>("SELECT count(*) AS n FROM fragrances").get();
check.close();

console.log(`backup ok: ${dest} (${snapshot.byteLength} bytes, ${row?.n} fragrances)`);
