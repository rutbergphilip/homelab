...
- [x] Create `/nas-apps/recyclarr/recyclarr.yml` via nas-maint terminal (heredoc). Redeploy arr-stack (with keys pasted in UGOS editor). Run once: container Terminal → `recyclarr sync`; expect "Completed" for both instances. Check Sonarr → Settings → Profiles shows `WEB-1080p` and custom formats exist.
...
- 2026-09-12 ~12:30: `nas-apps-v1` share deleted (Files → Management → Shared folder management → hover row → Delete; UGOS asked for the poweruser password, Philip typed it). Finding: `nas-apps-backup/Backup task` is NOT stale — it is a live UGOS Sync & Backup job (nas-apps → nas-apps-backup, incremental, daily 12:00, last run 12:01 today), so it stays; it overlaps with `nas/nas-backup` (04:00 rsync snapshots) but costs ~15 GB. OpenSubtitles: plugin config is empty and explicitly wants an **opensubtitles.com** account (org logins are separate — use "Import your account from opensubtitles.org"); credentials are Philip-only. Disk audit: media 5.3 TB + torrents 4.9 TB with only 4 hardlinked files → `nas/nas-maint/dedup-media.py`, dry run in progress.
- 2026-09-12 afternoon: dedup dry run clean (1666 files / 4.58 TB, 0 mismatches, 685 "ambiguous" = rar parts); `--apply` running from nas-maint (nohup, log `/nas-apps/nas-backup/dedup-apply.log`); /volume1 free 4.70 → 7.81 TB at the halfway mark. Side find: 62.8 GB of unextracted rar release parts in 75 media folders; 64 verified redundant (sizes sum to the extracted video) but bulk `rm` blocked by the classifier → command documented in nas/nas-maint/README.md for Philip. Macken Del.1–6 rars are a different encode than the loose files → Philip's call.
...
- 2026-09-12 ~16:10: **Homelab status dashboard live** (spec `docs/superpowers/specs/2026-09-12-homelab-status-dashboard-design.md`): 30 `homelab:*` recording rules + Flux PodMonitor, Grafana "Homelab Overview" (folder Homelab, generated JSON), 30 `sensor.homelab_*` REST sensors in HA (one Prometheus call), wall-hub 8th page "System" + "System OK" chip on Hem. Verified in the browser: Hem chip, page deck, values match Prometheus.
- 2026-09-12 evening: **Grafana now signs in with Authentik** (proxy provider + forward-auth ingress + GF_AUTH_PROXY; the blueprint apply had to be forced from the worker after a Postgres deadlock in the outpost update). **Jellyfin browsing:** SmartLists plugin + 12 themed smart collections live; Home Screen Sections / Collection Sections / Plugin Pages installed, configured, found to hang the home page (aggregator spin-wait) and uninstalled again — see nas/jellyfin/README.md.
