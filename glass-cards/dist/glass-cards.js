...
`;let $t=class extends ht{constructor(){super(...arguments),this._isOpen=!1,this._isClosing=!1,this._cards=[],this._onHashChange=()=>{this._checkHash()}}connectedCallback(){super.connectedCallback(),window.addEventListener("hashchange",this._onHashChange),this._checkHash()}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("hashchange",this._onHashChange)}_checkHash(){if(!this._config?.hash)return;const t=window.location.hash.replace("#","");t!==this._config.hash||this._isOpen?t!==this._config.hash&&this._isOpen&&this._close():this._open()}_open(){this._isOpen=!0,this._isClosing=!1,this._createPopupCards()}_close(){this._isClosing=!0,setTimeout(()=>{this._isOpen=!1,this._isClosing=!1},350),window.location.hash.replace("#","")===this._config.hash&&history.replaceState(null,"",window.location.pathname+window.location.search)}_handleBackdropClick(){this._close()}setConfig(t){if(!t.hash)throw new Error('glass-popup requires a "hash" property');this._config=t}set hass(t){this._hass=t,this._cards.forEach(e=>{e.hass=t})}get hass(){return this._hass}_createPopupCards(){this._config?.cards&&(this._cards=this._config.cards.map(t=>{const e=t.type?.startsWith("custom:")?t.type.replace("custom:",""):`hui-${t.type}-card`,i=document.createElement(e);return"function"==typeof i.setConfig&&i.setConfig(t),this.hass&&(i.hass=this.hass),i}),this.requestUpdate())}render(){return this._isOpen||this._isClosing?V`
    ...
    `:V``}getCardSize(){return 0}};$t.styles=[kt,n`
      :host { display: block; }
      .overlay {
        position: fixed;
        inset: 0;
  ...
  `,t([gt({attribute:!1})],Et.prototype,"hass",void 0),t([gt({attribute:!1})],Et.prototype,"_config",void 0),t([bt()],Et.prototype,"_activeHash",void 0),Et=t([dt("glass-nav-bar")],Et);let Ct=class extends vt{get _vacuumConfig(){return this._config}setConfig(t){super.setConfig(t),t.entity&&this.setTrackedEntities([t.entity])}_getStatusText(t){return{cleaning:"Stader",docked:"Dockad",paused:"Pausad",returning:"Atergar",idle:"Inaktiv",error:"Fel",unavailable:"Otillganglig"}[t]??t}_start(){this._config.entity&&this.callService("vacuum","start",void 0,this._config.entity)}_stop(){this._config.entity&&this.callService("vacuum","return_to_base",void 0,this._config.entity)}_cleanRoom(t){this._config.entity&&null!=t.room_id&&this.callService("vacuum","send_command",{command:"app_segment_clean",params:[t.room_id]},this._config.entity)}render(){if(!this.hass||!this._config?.entity)return V``;const t=this.getEntity(this._config.entity);if(!t)return V``;const e=t.state,i="cleaning"===e,a="error"===e,s=t.attributes.battery_level,r=this._config.name??t.attributes.friendly_name??"Vacuum",n=this._config.icon??"mdi:robot-vacuum";return V`
      <div
    ...
    `],Mt=t([dt("glass-info-row")],Mt);let St=class extends ht{set hass(t){}setConfig(t){if(!t.label)throw new Error('glass-section requires a "label" property');this._config=t}render(){return this._config?V`
      <div class="section">
        ${this._config.icon?V`<ha-icon .icon=${this._config.icon}></ha-icon>`:""}
        <span class="label">${this._config.label}</span>
      </div>
    `:V``}getCardSize(){return 0}};St.styles=n`
    :host { display: block; }
    .section {
      display: flex;
      align-items: center;
...
`,Pt="glass-hub-theme";const Nt="glass-hub-weather-bg";function zt(){return"off"!==localStorage.getItem(Nt)}function Ft(t){localStorage.setItem(Nt,t?"on":"off")}let Dt=null;function Bt(){return Dt}function Ot(t,e=8){return Math.abs(t)>e}const jt=t=>X`
...
`,It={lamp:jt(X`
  ...
  `,t([gt({attribute:!1})],Jt.prototype,"entity",void 0),t([gt({attribute:!1})],Jt.prototype,"theme",void 0),t([gt({attribute:!1})],Jt.prototype,"active",void 0),t([bt()],Jt.prototype,"_skyA",void 0),t([bt()],Jt.prototype,"_skyB",void 0),t([bt()],Jt.prototype,"_frontA",void 0),customElements.define("hub-weather-bg",Jt);const te='fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"',ee=X`<svg viewBox="0 0 24 24" ${te}>
  <circle cx="12" cy="12" r="4.2"></circle>
  <path d="M12 2.5v2.2M12 19.3v2.2M2.5 12h2.2M19.3 12h2.2M5 5l1.6 1.6M17.4 17.4L19 19M5 19l1.6-1.6M17.4 6.6L19 5"></path>
</svg>`,ie=X`<svg viewBox="0 0 24 24" ${te}>
  <path d="M19.5 14.2A7.8 7.8 0 0 1 9.8 4.5a7.8 7.8 0 1 0 9.7 9.7z"></path>
</svg>`,ae=X`<svg viewBox="0 0 24 24" ${te}>
  <path d="M6.5 17.5a4 4 0 0 1-.4-8 5.2 5.2 0 0 1 10.1-1.2 3.9 3.9 0 0 1 1.3 7.6 4 4 0 0 1-1.5.3z"></path>
</svg>`,se=X`<svg viewBox="0 0 24 24" ${te}>
...
</svg>`,re=X`<svg viewBox="0 0 24 24" ${te}>
  <path d="M20.6 8.6a4 4 0 0 1-5-5 4 4 0 1 0 5 5z"></path>
  <path d="M5.8 19a3.6 3.6 0 0 1-.3-7.2 4.7 4.7 0 0 1 9.1-1.1 3.5 3.5 0 0 1 1.2 6.9 3.6 3.6 0 0 1-1.4.3z"></path>
</svg>`,ne=X`<svg viewBox="0 0 24 24" ${te}>
...
</svg>`,oe=X`<svg viewBox="0 0 24 24" ${te}>
...
</svg>`,ue=X`<svg viewBox="0 0 24 24" ${te}>
  ...
  <path d="M8 18v.01M12 18v.01M16 18v.01M10 21v.01M14 21v.01" stroke-width="2.6"></path>
    ...
    `],t([gt({attribute:!1})],xe.prototype,"weatherEntity",void 0),t([bt()],xe.prototype,"_now",void 0),t([bt()],xe.prototype,"_hours",void 0),t([bt()],xe.prototype,"_days",void 0),customElements.define("hub-clock",xe);class ye extends ht{constructor(){super(...arguments),this.icon="",this.label="",this.tone="neutral",this.active=!1}render(){const t=It[this.icon];return V`
      <span class="chip tone-${this.tone} ${this.active?"active":""}">
        ${t?V`<span class="icon">${t}</span>`:""}
        <span class="label">${this.label}</span>
      </span>
    `}}function _e(t,e){const i=t.lights.some(t=>"on"===e[t.entity]?.state);if(i)return{service:"turn_off",entities:t.lights.map(t=>t.entity)};return{service:"turn_on",entities:t.default_lights?.length?t.default_lights:[t.main_entity]}}ye.styles=[Tt,n`
      :host {
        display: inline-flex;
      }
      .chip {
    ...
    `],t([gt({attribute:!1})],ye.prototype,"icon",void 0),t([gt({attribute:!1})],ye.prototype,"label",void 0),t([gt({attribute:!1})],ye.prototype,"tone",void 0),t([gt({type:Boolean})],ye.prototype,"active",void 0),customElements.define("hub-status-chip",ye);class we extends vt{constructor(){super(...arguments),this._longPressed=!1,this._downX=0,this._downY=0,this._flash=!1,this._onPointerDown=t=>{this._longPressed=!1,this._downX=t.clientX,this._downY=t.clientY,this._pressTimer=window.setTimeout(()=>{this._longPressed=!0,this.dispatchEvent(new CustomEvent("hub-room-open",{detail:{roomId:this.room.id},bubbles:!0,composed:!0}))},500)},this._onPointerMove=t=>{void 0!==this._pressTimer&&(Ot(t.clientX-this._downX)||Ot(t.clientY-this._downY))&&this._cancelPress()},this._cancelPress=()=>{void 0!==this._pressTimer&&(clearTimeout(this._pressTimer),this._pressTimer=void 0)},this._onClick=()=>{if(this._longPressed)return void(this._longPressed=!1);const t=_e(this.room,this.hass.states);this.callService("light",t.service,{entity_id:t.entities}),this._flash=!0,void 0!==this._flashTimer&&clearTimeout(this._flashTimer),this._flashTimer=window.setTimeout(()=>{this._flash=!1,this._flashTimer=void 0},200)}}disconnectedCallback(){super.disconnectedCallback(),this._cancelPress(),void 0!==this._flashTimer&&(clearTimeout(this._flashTimer),this._flashTimer=void 0)}get _lightsOn(){return this.room.lights.filter(t=>this.isOn(t.entity)).length}get _brightnessPct(){const t=this.getEntityAttribute(this.room.main_entity,"brightness");return"number"==typeof t?Math.round(t/255*100):null}get _subtitle(){const t=this._lightsOn;if(0===t)return"Släckt";const e=1===t?"1 lampa":`${t} lampor`,i=this._brightnessPct;return null!==i?`${e} · ${i} %`:e}render(){if(!this.hass||!this.room)return V``;const t=this._lightsOn>0,e=It[this.room.icon];return V`
      <div
        class="tile ${t?"active":""} ${this._flash?"flash":""}"
    ...
    `],t([gt({attribute:!1})],we.prototype,"room",void 0),t([bt()],we.prototype,"_flash",void 0),customElements.define("hub-room-tile",we);const ke=new Set(["off","unavailable","unknown","standby","idle"]);function $e(t,e){for(const i of e){const e=t[i.entity];if(e&&"playing"===e.state)return{entity:e,name:i.name}}for(const i of e){const e=t[i.entity];if(e&&!ke.has(e.state))return{entity:e,name:i.name}}return null}function Ee(t,e){if(!t)return 0;const i=t.attributes,a="number"==typeof i.media_duration?i.media_duration:0;if(a<=0)return 0;let s="number"==typeof i.media_position?i.media_position:0;const r="string"==typeof i.media_position_updated_at?Date.parse(i.media_position_updated_at):NaN;return"playing"!==t.state||Number.isNaN(r)||(s+=(e-r)/1e3),Math.max(0,Math.min(100,s/a*100))}class Ce extends vt{constructor(){super(...arguments),this.players=[],this._now=Date.now()}connectedCallback(){super.connectedCallback(),this._interval=window.setInterval(()=>{this._now=Date.now()},1e3)}disconnectedCallback(){super.disconnectedCallback(),void 0!==this._interval&&(clearInterval(this._interval),this._interval=void 0)}_goto(){this.dispatchEvent(new CustomEvent("hub-goto-page",{detail:{page:"media"},bubbles:!0,composed:!0}))}_togglePlay(t,e){t.stopPropagation(),this.callService("media_player","media_play_pause",void 0,e)}render(){if(!this.hass)return V``;const t=$e(this.hass.states,this.players??[]);if(!t)return V`
        <div class="np idle" @click=${this._goto}>
          <span class="idle-ic">${It.note}</span>
          <b class="title dim">Ingenting spelas</b>
        </div>
      `;const e=t.entity,i="playing"===e.state,a=e.attributes.media_title||t.name,s=e.attributes.media_artist||t.name,r=e.attributes.entity_picture,n=Ee(e,this._now);return V`
      <div class="np ${i?"playing":""}" @click=${this._goto}>
        ...
        >
          <span class="ppic">${i?It.pause:It.play}</span>
        </button>
      </div>
    `}}Ce.styles=[Tt,n`
      :host {
    ...
    `],t([gt({attribute:!1})],Ce.prototype,"players",void 0),t([bt()],Ce.prototype,"_now",void 0),customElements.define("hub-now-playing",Ce);const Me=new Intl.NumberFormat("sv-SE");function Se(t,e){return e>0?Math.max(0,Math.min(100,t/e*100)):0}class Ae extends vt{_goto(){this.dispatchEvent(new CustomEvent("hub-goto-page",{detail:{page:"kcal"},bubbles:!0,composed:!0}))}render(){if(!this.hass)return V``;const t=this.todayEntity?this.getEntity(this.todayEntity):void 0,e=t?Number(t.state):NaN;if(!t||"unavailable"===t.state||"unknown"===t.state||Number.isNaN(e))return V`
        <div class="kc offline" @click=${this._goto}>
          <div class="ring" style="--pct:0"></div>
          <div class="meta"><b class="val">Kcal · offline</b></div>
        </div>
      `;const i="number"==typeof t.attributes.kcal_target?t.attributes.kcal_target:0,a=Se(e,i),s=function(t){const e=t.protein_g;return"number"==typeof e?`${Math.round(e)} g protein`:""}(t.attributes);return V`
      <div class="kc" @click=${this._goto}>
        <div class="ring" style="--pct:${a}"></div>
        <div class="meta">
          <b class="val">
            ${Me.format(Math.round(e))}
            <span class="target">
              ${i>0?`/ ${Me.format(i)} kcal`:"kcal"}
            </span>
          </b>
    ...
    `],t([gt({attribute:!1})],Ae.prototype,"todayEntity",void 0),customElements.define("hub-kcal-ring",Ae);const Te={frukost:"Frukost",lunch:"Lunch",middag:"Middag",mellis:"Mellis"},Pe=["frukost","lunch","middag","mellis"],Ne={gymdag:"G",vilodag:"V",flexdag:"F"};const ze=/^\d{4}-\d{2}-\d{2}$/;function Fe(t){if(!t||"object"!=typeof t)return null;const e=t;if("frukost"!==(i=e.slot)&&"lunch"!==i&&"middag"!==i&&"mellis"!==i||"string"!=typeof e.name||""===e.name)return null;var i;const a=t=>"number"==typeof t&&Number.isFinite(t)?t:0;return{slot:e.slot,name:e.name,kcal:a(e.kcal),protein:a(e.protein),fat:a(e.fat),carbs:a(e.carbs),logged:!0===e.logged}}function De(t){if(!t||"object"!=typeof t)return null;const e=t;if("string"!=typeof e.date||!ze.test(e.date))return null;const i=Array.isArray(e.meals)?e.meals.map(Fe).filter(t=>null!==t):[];return{date:e.date,weekday:"string"==typeof e.weekday?e.weekday:"",day_type:"string"==typeof e.day_type?e.day_type:"vilodag",confirmed:!0===e.confirmed,meals:i,total_kcal:"number"==typeof e.total_kcal?e.total_kcal:0,target_kcal:"number"==typeof e.target_kcal?e.target_kcal:0,protein_ok:!0===e.protein_ok,kcal_ok:!1!==e.kcal_ok}}function Be(t){if(!t)return null;const{week_start:e,today:i,days:a}=t;if("string"!=typeof e||!ze.test(e))return null;if(!Array.isArray(a))return null;const s=a.map(De).filter(t=>null!==t);return 0===s.length?null:{weekStart:e,today:"string"==typeof i&&ze.test(i)?i:e,confirmedDays:s.filter(t=>t.confirmed).length,days:s}}function Oe(t){const e=t=>Pe.flatMap(e=>t.meals.filter(t=>t.slot===e&&!t.logged)),i=t.days.find(e=>e.date===t.today);if(i){const t=e(i);if(t.length>0)return{dayLabel:"Idag",day:i,meals:t}}const a=t.days.find(e=>e.date===function(t,e){const[i,a,s]=t.split("-").map(Number);return new Date(Date.UTC(i,a-1,s+e)).toISOString().slice(0,10)}(t.today,1));if(a){const t=e(a);if(t.length>0)return{dayLabel:"Imorgon",day:a,meals:t}}return null}const je=new Intl.NumberFormat("sv-SE");class Ie extends vt{_model(){if(!this.plannerEntity)return null;const t=this.getEntity(this.plannerEntity);return t&&"unavailable"!==t.state&&"unknown"!==t.state?Be(t.attributes):null}_open(){this.dispatchEvent(new CustomEvent("hub-goto-page",{detail:{page:"vecka"},bubbles:!0,composed:!0}))}render(){if(!this.hass||!this.plannerEntity)return q;const t=this._model(),e=t?Oe(t):null;return V`
      <div class="card${e?"":" empty"}" @click=${this._open}>
        <div class="head">
          <span class="eyebrow">Matsedel</span>
          ${t?V`<span class="count">${t.confirmedDays} / 7 ✓</span>`:q}
        </div>
        ${e?V`
              ...
              <div class="meals">
                ${e.meals.map(t=>V`
                    <div class="meal">
                      <span class="slot">${Te[t.slot]}</span>
    ...
    `}}function He(t){const e=Date.parse(t.expected??t.scheduled??"");return Number.isNaN(e)?Number.POSITIVE_INFINITY:e}function Le(t,e,i){if(!Array.isArray(t))return[];const a=i?new RegExp(i,"i"):null;return t.filter(t=>t?.line?.designation===e).filter(t=>!(a&&t.destination&&a.test(t.destination))).sort((t,e)=>He(t)-He(e))}Ie.styles=[Tt,n`
      :host {
        display: block;
        height: 100%;
      }
    ...
    `],t([gt({attribute:!1})],Ie.prototype,"plannerEntity",void 0),customElements.define("hub-meal-card",Ie);function Re(t){if(!Array.isArray(t))return[];const e=new Map;for(const i of t){if(!i||"object"!=typeof i)continue;const t=i;if("string"!=typeof t.header||0===t.header.length)continue;const a="number"==typeof t.priority?t.priority:0,s=Array.isArray(t.lines)?t.lines.map(t=>t&&"object"==typeof t?t.designation:null).filter(t=>"string"==typeof t&&t.length>0):[],r="string"==typeof t.details&&t.details.length>0?t.details:void 0,n="string"==typeof t.scope&&t.scope.length>0?t.scope:void 0,o=e.get(t.header);if(o){for(const t of s)o.badges.add(t);o.priority=Math.max(o.priority,a),!o.details&&r&&(o.details=r),!o.scope&&n&&(o.scope=n)}else e.set(t.header,{badges:new Set(s),priority:a,details:r,scope:n})}return[...e.entries()].sort((t,e)=>e[1].priority-t[1].priority).slice(0,5).map(([t,e])=>({badges:[...e.badges].sort(),header:t,...void 0!==e.details?{details:e.details}:{},...void 0!==e.scope?{scope:e.scope}:{}}))}const Ue=new Set(["unavailable","unknown",""]);function Ve(t){if(!t||Ue.has(t))return null;const e=new Date(t);if(Number.isNaN(e.getTime()))return null;return`${String(e.getHours()).padStart(2,"0")}:${String(e.getMinutes()).padStart(2,"0")}`}class Xe extends vt{constructor(){super(...arguments),this._open=()=>{this.dispatchEvent(new CustomEvent("hub-transit-open",{bubbles:!0,composed:!0}))}}_pendeltag(){const t=this.config.transit?.pendeltag;if(!t)return V`<span class="sub dim">–</span>`;const e=this.getEntity(t.next_entity),i=Ve(e?.state);if(!i)return V`<span class="sub dim">–</span>`;const a=this.getEntity(t.count_entity),s=a&&!Number.isNaN(Number(a.state))?Number(a.state):null;return V`<span class="sub">Nästa ${i}${null===s?"":` · ${s} ${1===s?"avgång":"avgångar"}`}</span>`}_bus(){const t=this.config.transit?.bus;if(!t)return V`<span class="sub dim">Inga avgångar idag</span>`;const e=this.getEntity(t.entity),i=Le(e?.attributes.departures??[],t.line,t.exclude_destination).slice(0,3);return 0===i.length?V`<span class="sub dim">Inga avgångar idag</span>`:V`<span class="sub"
      >${i.map((t,e)=>V`${e>0?V`<span class="sep">·</span>`:""}<span
            class="dep ${t.state&&"EXPECTED"!==t.state?"delayed":""}"
            >${t.display??"–"}</span
          >`)}</span
    >`}_shaped(){const t=this.config.disturbances_entity?this.getEntity(this.config.disturbances_entity):void 0;return t&&"unavailable"!==t.state&&"unknown"!==t.state?Re(t.attributes.deviations):[]}_alerts(t){if(0===t.length)return q;if(1===t.length){const e=t[0];return V`<div class="alerts">
        <div class="alert">
    ...
    `],t([gt({attribute:!1})],Xe.prototype,"config",void 0),customElements.define("hub-transit-card",Xe);const We=36e5,qe=new Set(["unavailable","unknown","none",""]);function Ge(t,e,i){if(!Array.isArray(t))return[];const a=[];for(const s of t){if(!s||"object"!=typeof s)continue;const t=s,r="number"==typeof t.total?t.total:Number(t.total);if(!Number.isFinite(r)||"string"!=typeof t.startsAt)continue;const n=new Date(t.startsAt);if(Number.isNaN(n.getTime()))continue;const o="number"==typeof t.energy?t.energy:Number(t.energy),l=Number.isFinite(o)?100*o:null,h=100*r,c="spot"===e&&null!==l?l:h+("allin"===e?i:0);a.push({start:n,ore:c,totalOre:h,spotOre:l})}return a.sort((t,e)=>t.start.getTime()-e.start.getTime())}function Ye(t){if(t.length<3)return null;let e=1/0,i=-1;for(let a=0;a+3<=t.length;a++){let s=!0,r=t[a].ore;for(let e=a+1;e<a+3;e++){if(t[e].start.getTime()-t[e-1].start.getTime()!==We){s=!1;break}r+=t[e].ore}s&&r<e&&(e=r,i=a)}return i<0?null:{start:t[i].start,end:new Date(t[i+3-1].start.getTime()+We)}}function Ke(t,e,i,a="allin",s=0){if(qe.has(String(e??"").toLowerCase()))return{now:null,level:"normal",today:[],tomorrow:[],cheapestWindow:null};const r=Ge(t?.today,a,s),n=Ge(t?.tomorrow,a,s);if(0===r.length&&0===n.length)return{now:null,level:"normal",today:[],tomorrow:[],cheapestWindow:null};const o=[...r,...n].sort((t,e)=>t.start.getTime()-e.start.getTime()),l=i.getTime(),h=o.find(t=>t.start.getTime()<=l&&l<t.start.getTime()+We)??null;let c="normal";if(h&&r.length){const t=r.reduce((t,e)=>t+e.ore,0)/r.length;if(t>0){const e=h.ore/t;e<.85?c="låg":e>1.15&&(c="hög")}}const d=o.filter(t=>t.start.getTime()+We>l);return{now:h,level:c,today:r,tomorrow:n,cheapestWindow:Ye(d)}}function Ze(t){const e=[...t.today,...t.tomorrow];return e.length>0&&e.every(t=>null!==t.spotOre)}const Qe="glass-hub-price-view";function Je(){try{return"spot"===localStorage.getItem(Qe)?"spot":"allin"}catch{return"allin"}}function ti(t){return(t?.grid?.overforing_ore??0)+(t?.grid?.energiskatt_ore??0)}const ei={"låg":"lågt",normal:"normalt","hög":"högt"};class ii extends vt{constructor(){super(...arguments),this._now=new Date,this._open=()=>{this.dispatchEvent(new CustomEvent("hub-goto-page",{detail:{page:"energi"},bubbles:!0,composed:!0}))}}connectedCallback(){super.connectedCallback(),this._interval=window.setInterval(()=>{this._now=new Date},6e4)}disconnectedCallback(){super.disconnectedCallback(),void 0!==this._interval&&(clearInterval(this._interval),this._interval=void 0)}_model(){const t=this.config.price_series_entity?this.getEntity(this.config.price_series_entity):void 0;if(!t)return null;const e=Je(),i=t.attributes,a=Ke(i,t.state,this._now,e,"allin"===e?ti(this.config):0);return"spot"===e&&a.today.length&&a.today.some(t=>null===t.spotOre)?Ke(i,t.state,this._now,"allin",ti(this.config)):a}_currentOre(t){if(t?.now)return Math.round(t.now.ore);const e=this.config.price_entity?this.getEntity(this.config.price_entity):void 0;return e&&!Number.isNaN(Number(e.state))?Math.round(100*Number(e.state)):null}_bars(t){const e=function(t,e){const i=[...t.today,...t.tomorrow].sort((t,e)=>t.start.getTime()-e.start.getTime()),a=e.getTime(),s=t.cheapestWindow,r=s?s.start.getTime():null,n=s?s.end.getTime():null;return i.filter(t=>t.start.getTime()+We>a).slice(0,12).map(t=>{const e=t.start.getTime();return{start:t.start,ore:t.ore,current:e<=a&&a<e+We,cheap:null!==r&&e>=r&&e<n}})}(t,this._now);if(0===e.length)return V`<div class="waiting">Väntar på prisdata</div>`;const i=e.map(t=>t.ore),a=Math.min(...i),s=Math.max(...i)-a;return V`<div class="bars">
      ${e.map(t=>{return V`<div
        ...
        ></div>`;var e})}
          ...
          ><span class="ic">${It.clock}</span>Billigast ${n.start.getHours()}–${n.end.getHours()}</span
        >`:q;return V`
      <button class="card" aria-label=${null===e?"Elpris, öppna energisidan":`Elpris ${e} öre just nu${r?`, ${ei[a]}`:""}, öppna energisidan`} @click=${this._open}>
        <div class="head">
          <div class="lead">
            <span class="ic">${It.bolt}</span>
            <span class="num ${s}">${null===e?"—":e}</span>
            <span class="unit">öre</span>
            ${r?V`<span class="level ${s}">· ${ei[a]}</span>`:q}
          </div>
          ${o}
        </div>
    ...
    `],t([gt({attribute:!1})],ii.prototype,"config",void 0),t([bt()],ii.prototype,"_now",void 0),customElements.define("hub-energy-strip",ii);const ai={cleaning:"Städar",returning:"Åker hem",paused:"Pausad",error:"Fel",idle:"Väntar"};class si extends vt{constructor(){super(...arguments),this.theme="natt",this.weatherBg=!1,this.pageActive=!1}_gotoPage(t){this.dispatchEvent(new CustomEvent("hub-goto-page",{detail:{page:t},bubbles:!0,composed:!0}))}get _chips(){const t=this.config,e=[],i=t.lights_count_entity?this.getEntity(t.lights_count_entity):void 0,a=i&&!Number.isNaN(Number(i.state))?Number(i.state):null;if(e.push({icon:"lamp",label:null===a?"—":`${a} ${1===a?"lampa":"lampor"}`,tone:"amber",active:(a??0)>0}),t.vacuum_entity){const i=this.getEntity(t.vacuum_entity);i&&"docked"!==i.state&&"unavailable"!==i.state&&"unknown"!==i.state&&e.push({icon:"vacuum",label:ai[i.state]??"Städar",tone:"error"===i.state?"coral":"neutral",active:!0})}if(t.person_entity){const i=this.getEntity(t.person_entity),a=(i?.attributes.friendly_name||"Philip").split(" ")[0],s="home"===i?.state;e.push({icon:"home",label:`${a} ${s?"hemma":"borta"}`,tone:"neutral",active:!1})}return e}render(){if(!this.hass||!this.config)return V``;const t=this.config;return V`
          ...
          <div class="chips">
            ${this._chips.map(t=>V`
                <hub-status-chip
                  ...
                  .tone=${t.tone}
                  ?active=${t.active}
                  @click=${t.goto?()=>this._gotoPage(t.goto):q}
                ></hub-status-chip>
              `)}
          </div>
        </div>
    ...
    `],t([gt({attribute:!1})],si.prototype,"config",void 0),t([gt({attribute:!1})],si.prototype,"theme",void 0),t([gt({attribute:!1})],si.prototype,"weatherBg",void 0),t([gt({attribute:!1})],si.prototype,"pageActive",void 0),customElements.define("hub-home-page",si);const ri=new Set(["unavailable","unknown"]);class ni extends vt{constructor(){super(...arguments),this._flash=!1,this._longPressed=!1,this._downX=0,this._downY=0,this._onPointerDown=t=>{this._dead||(this._longPressed=!1,this._downX=t.clientX,this._downY=t.clientY,this._pressTimer=window.setTimeout(()=>{this._longPressed=!0,this.dispatchEvent(new CustomEvent("hub-light-open",{detail:{entity:this.light.entity,name:this.light.name},bubbles:!0,composed:!0}))},500))},this._onPointerMove=t=>{void 0!==this._pressTimer&&(Ot(t.clientX-this._downX)||Ot(t.clientY-this._downY))&&this._cancelPress()},this._cancelPress=()=>{void 0!==this._pressTimer&&(clearTimeout(this._pressTimer),this._pressTimer=void 0)},this._onClick=()=>{this._dead||(this._longPressed?this._longPressed=!1:(this.callService("light","toggle",void 0,this.light.entity),this._flash=!0,void 0!==this._flashTimer&&clearTimeout(this._flashTimer),this._flashTimer=window.setTimeout(()=>{this._flash=!1,this._flashTimer=void 0},200)))}}disconnectedCallback(){super.disconnectedCallback(),this._cancelPress(),void 0!==this._flashTimer&&(clearTimeout(this._flashTimer),this._flashTimer=void 0)}get _dead(){const t=this.getEntity(this.light.entity);return!t||ri.has(t.state)}get _stateLabel(){const t=this.getEntity(this.light.entity);if(!t||ri.has(t.state))return"Ej tillgänglig";if("on"!==t.state)return"Av";const e=t.attributes.brightness;return"number"==typeof e?`${Math.round(e/255*100)} %`:"På"}render(){if(!this.hass||!this.light)return V``;const t=this.isOn(this.light.entity),e=this._dead;return V`
      <div
    ...
    `],t([gt({attribute:!1})],ni.prototype,"light",void 0),t([bt()],ni.prototype,"_flash",void 0),customElements.define("hub-light-tile",ni);const oi=new Set(["unavailable","unknown"]);function li(t){return!!t&&!oi.has(t.state)}class hi extends vt{constructor(){super(...arguments),this._armed=!1,this._flash=!1,this._headLongPressed=!1,this._headDownX=0,this._headDownY=0,this._onAllOff=()=>{if(!this._armed)return this._armed=!0,void 0!==this._armTimer&&clearTimeout(this._armTimer),void(this._armTimer=window.setTimeout(()=>{this._armed=!1,this._armTimer=void 0},3e3));void 0!==this._armTimer&&clearTimeout(this._armTimer),this._armTimer=void 0,this._armed=!1,this._flash=!0,this.callService("light","turn_off",void 0,"all"),void 0!==this._flashTimer&&clearTimeout(this._flashTimer),this._flashTimer=window.setTimeout(()=>{this._flash=!1,this._flashTimer=void 0},200)},this._onHeadMove=t=>{void 0!==this._headPressTimer&&(Ot(t.clientX-this._headDownX)||Ot(t.clientY-this._headDownY))&&this._cancelHeadPress()},this._cancelHeadPress=()=>{void 0!==this._headPressTimer&&(clearTimeout(this._headPressTimer),this._headPressTimer=void 0)}}disconnectedCallback(){super.disconnectedCallback(),this._clearTimers()}_clearTimers(){void 0!==this._armTimer&&clearTimeout(this._armTimer),void 0!==this._flashTimer&&clearTimeout(this._flashTimer),this._armTimer=void 0,this._flashTimer=void 0,this._armed=!1,this._flash=!1,this._cancelHeadPress()}_activateScene(t){this.callService("scene","turn_on",void 0,t)}_onHeadDown(t,e){this._headLongPressed=!1,this._headDownX=t.clientX,this._headDownY=t.clientY,this._headPressTimer=window.setTimeout(()=>{this._headLongPressed=!0,this.dispatchEvent(new CustomEvent("hub-room-open",{detail:{roomId:e.id},bubbles:!0,composed:!0}))},500)}_onHeadClick(t){if(this._headLongPressed)return void(this._headLongPressed=!1);const e=_e(t,this.hass.states);this.callService("light",e.service,{entity_id:e.entities})}_section(t){const e=function(t,e){const i=t.lights.filter(t=>"on"===e[t.entity]?.state),a=i.length;if(0===a)return{onCount:0,pct:null,label:"Släckt"};const s=i.map(t=>e[t.entity]?.attributes.brightness).filter(t=>"number"==typeof t),r=s.length?Math.round(s.reduce((t,e)=>t+e,0)/s.length/255*100):null,n=1===a?"1 lampa":`${a} lampor`;return{onCount:a,pct:r,label:null!==r?`${n} · ${r} %`:n}}(t,this.hass.states),i=e.onCount>0;return V`
      <div class="section">
        <div
    ...
    `}render(){if(!this.hass||!this.config)return V``;const t=this.config,e=function(t,e){let i=0,a=0;for(const s of t.rooms??[])for(const t of s.lights){const s=e[t.entity];li(s)&&(a+=1,"on"===s.state&&(i+=1))}return{on:i,total:a}}(t,this.hass.states);return V`
      <div class="page">
        <div class="header">
          <div class="heading">
          ...
          <div class="actions">
            ${(t.scenes??[]).map(t=>V`
                ...
                >
                  ${It[t.icon]?V`<span class="ic">${It[t.icon]}</span>`:q}
                  <span>${t.name}</span>
                </button>
              `)}
            <button
    ...
    `}render(){if(!this.model||0===this.model.today.length)return V``;const t=this._slots(),{min:e,max:i}=this._bounds(),a=t.map(t=>"divider"===t.kind?"8px":"minmax(0, 1fr)").join(" ");return V`
      <div class="chart">
        <div class="plot" style="grid-template-columns:${a}">
          ${t.map((a,s)=>{if("divider"===a.kind)return V`<div class="divider"></div>`;const r=this._height(a.hour.ore,e,i),n=a.cls.startsWith("future")?`background:${this._tint(a.hour.ore,e,i)}`:"";return V`
              <div
                class="cell ${a.cls}"
                style="--bar-h:${r}%"
                @click=${()=>this._toggleDetail(s)}
              >
                ${this._detail===s?this._flyout(a.hour,s,t.length):q}
                ${a.label&&this._detail!==s?V`<span class="cell-label">${a.label}</span>`:q}
    ...
    `],t([gt({attribute:!1})],ci.prototype,"model",void 0),t([gt({type:Number})],ci.prototype,"gridAddOre",void 0),t([bt()],ci.prototype,"_detail",void 0),customElements.define("hub-price-chart",ci);const di={"låg":"lågt",normal:"normalt","hög":"högt"};class pi extends vt{constructor(){super(...arguments),this._now=new Date,this._view=Je()}connectedCallback(){super.connectedCallback(),this._interval=window.setInterval(()=>{this._now=new Date},6e4)}disconnectedCallback(){super.disconnectedCallback(),void 0!==this._interval&&(clearInterval(this._interval),this._interval=void 0)}_model(){const t=this.config.price_series_entity?this.getEntity(this.config.price_series_entity):void 0;if(!t)return null;const e=t.attributes;let i=Ke(e,t.state,this._now,this._view,"allin"===this._view?ti(this.config):0);return"spot"!==this._view||Ze(i)||(i=Ke(e,t.state,this._now,"allin",ti(this.config))),i}_setView(t){this._view=t,function(t){try{localStorage.setItem(Qe,t)}catch{}}(t)}_currentOre(t){if(t?.now)return Math.round(t.now.ore);const e=this.config.price_entity?this.getEntity(this.config.price_entity):void 0;return e&&!Number.isNaN(Number(e.state))?Math.round(100*Number(e.state)):null}_chips(t){const e=this.config,i=[],a=e.co2_entity?this.getEntity(e.co2_entity):void 0;a&&!Number.isNaN(Number(a.state))&&i.push({icon:"leaf",label:`${Math.round(Number(a.state))} g CO₂`,tone:"green"});const s=e.fossil_entity?this.getEntity(e.fossil_entity):void 0;if(s&&!Number.isNaN(Number(s.state))){const t=Math.round(Number(s.state));i.push({icon:"leaf",label:`${t} % fossilt`,tone:t>=40?"coral":"green"})}const r=t?.cheapestWindow;if(r){const t=r.start.getHours(),e=r.end.getHours();i.push({icon:"clock",label:`Billigast ${t}–${e}`,tone:"green"})}return i}render(){if(!this.hass||!this.config)return V``;const t=this._model(),e=this._currentOre(t),i=t?.now?t.level:"normal",a=!!t&&t.today.length>0,s=this._chips(t),r=!!t&&Ze(t),n="låg"===i?"low":"hög"===i?"high":"",o=!!t?.now&&"normal"!==i;return V`
      <div class="page">
        <div class="header">
          <div class="head-row">
            <div>
              ...
              <div class="subline">
                ${"spot"===this._view?"spotpris":"allt-in"} just nu${o?V` ·
        ...
        <div class="chart-wrap">
          ${a?V`<hub-price-chart .model=${t} .gridAddOre=${ti(this.config)}></hub-price-chart>`:V`<div class="waiting">Väntar på prisdata</div>`}
        </div>
    ...
    `],t([gt({attribute:!1})],bi.prototype,"player",void 0),t([gt({attribute:!1})],bi.prototype,"groupMaster",void 0),t([bt()],bi.prototype,"_drag",void 0),customElements.define("hub-volume-row",bi);const mi=new Set(["off","unavailable","unknown","standby","idle"]);function vi(t){const e=Number.isFinite(t)&&t>0?t:0,i=Math.floor(e/60),a=Math.floor(e%60);return`${i}:${String(a).padStart(2,"0")}`}class fi extends vt{constructor(){super(...arguments),this._sel=null,this._rgb=null,this._now=Date.now()}connectedCallback(){super.connectedCallback(),this._interval=window.setInterval(()=>{this._now=Date.now()},1e3)}disconnectedCallback(){super.disconnectedCallback(),void 0!==this._interval&&(clearInterval(this._interval),this._interval=void 0)}get _players(){return this.config?.media_players??[]}_selId(){if(this._sel)return this._sel;const t=$e(this.hass?.states??{},this._players);return t?.entity.entity_id??this._players[0]?.entity??null}_theme(){const t=this.getRootNode()?.host;return"dag"===t?.getAttribute("data-theme")?"dag":"natt"}updated(t){const e=this._selId(),i=e?this.hass?.states[e]?.attributes.entity_picture:void 0;i!==this._pic&&(this._pic=i,i?ui(i).then(t=>{this._pic===i&&(this._rgb=t)}):this._rgb=null)}_transport(t,e){this.callService("media_player",t,void 0,e)}_hero(t,e){const i="playing"===t.state,a=t.attributes.media_title||e,s=t.attributes.media_artist||e,r=t.attributes.entity_picture,n="number"==typeof t.attributes.media_duration?t.attributes.media_duration:0,o=Ee(t,this._now),l=o/100*n,h=t.entity_id;return V`
      <div class="hero">
    ...
    `],t([gt({attribute:!1})],fi.prototype,"config",void 0),t([bt()],fi.prototype,"_sel",void 0),t([bt()],fi.prototype,"_rgb",void 0),t([bt()],fi.prototype,"_now",void 0),customElements.define("hub-media-page",fi);let xi=0;class yi extends ht{constructor(){super(...arguments),this.points=[],this.stroke="--hub-lavender",this.width=560,this.height=130,this._gid="hub-spark-"+xi++}render(){const t=function(t,e,i,a=.1){const s=t.length;if(0===s)return[];if(1===s)return[{x:e,y:i/2}];const r=t.map(t=>t.value),n=Math.min(...r),o=Math.max(...r)-n,l=n-o*a,h=o*(1+2*a);return t.map((t,a)=>({x:a/(s-1)*e,y:o<=0?i/2:i-(t.value-l)/h*i}))}(this.points,this.width,this.height);if(0===t.length)return V``;const e=t.map(t=>`${t.x.toFixed(2)},${t.y.toFixed(2)}`).join(" "),i=t[t.length-1],a=t[0],s=t.length>=2,r=`${e} ${i.x.toFixed(2)},${this.height} ${a.x.toFixed(2)},${this.height}`;return V`
  ...
  `,t([gt({attribute:!1})],yi.prototype,"points",void 0),t([gt()],yi.prototype,"stroke",void 0),t([gt({type:Number})],yi.prototype,"width",void 0),t([gt({type:Number})],yi.prototype,"height",void 0),customElements.define("hub-sparkline",yi);const _i=new Intl.NumberFormat("sv-SE"),wi=new Intl.NumberFormat("sv-SE",{minimumFractionDigits:1,maximumFractionDigits:1}),ki=new Intl.NumberFormat("sv-SE",{maximumFractionDigits:1}),$i=new Intl.DateTimeFormat("sv-SE",{day:"numeric",month:"short",timeZone:"UTC"}),Ei=new Intl.DateTimeFormat("sv-SE",{weekday:"long",day:"numeric",month:"long",timeZone:"UTC"});function Ci(t){if(!t)return"";const e=new Date(`${t}T00:00:00Z`);return Number.isNaN(e.getTime())?"":$i.format(e).replace(/\.$/,"")}class Mi extends vt{_meals(t){const e=t.attributes.meals;return Array.isArray(e)?e.filter(t=>!!t&&"object"==typeof t).map(t=>({name:"string"==typeof t.name?t.name:"",kcal:"number"==typeof t.kcal?t.kcal:Number(t.kcal)||0})).filter(t=>t.name):[]}_num(t){return"number"==typeof t?t:NaN}_offline(){return V`
      <div class="page">
        <div class="offline">
    ...
    `}_weightCard(){const t=this.config.kcal?.forecast_entity,e=t?this.getEntity(t):void 0,i=e?Number(e.state):NaN;if(!e||"unavailable"===e.state||"unknown"===e.state||Number.isNaN(i))return V`
      ...
      `;const a=e.attributes.weight_trend,s=Array.isArray(a)?a.filter(t=>!!t&&"object"==typeof t).map(t=>({date:String(t.date??""),value:Number(t.kg)})).filter(t=>Number.isFinite(t.value)):[],r=function(t){if(t.length<2)return null;const e=new Date(`${t[0].date}T00:00:00Z`).getTime(),i=new Date(`${t[t.length-1].date}T00:00:00Z`).getTime();return Number.isNaN(e)||Number.isNaN(i)?null:Math.round((i-e)/864e5)}(s),n=s.length>=2?s[s.length-1].value-s[0].value:null,o=null===n||null===r?null:`${n<0?"−":n>0?"+":""}${wi.format(Math.abs(n))} kg på ${r} ${1===r?"dag":"dagar"}`,l=e.attributes.forecast,h=l&&"object"==typeof l?l:null,c=h?function(t){const e="number"==typeof t.goal_kg?`Mål ${ki.format(t.goal_kg)} kg`:"",i=t.eta?Ci(t.eta):"",a=t.eta_early&&t.eta_late?`${Ci(t.eta_early)}–${Ci(t.eta_late)}`:"";return[e,i?`ETA ${i}${a?` (${a})`:""}`:""].filter(Boolean).join(" · ")}(h):"",d=!!h?.on_track;return V`
      <section class="card">
        <span class="w-eyebrow">Vikt</span>
        <div class="w-num-row">
          <span class="w-num">${wi.format(i)}</span>
    ...
    `}render(){if(!this.hass||!this.config)return V``;const t=this.config.kcal?.today_entity,e=t?this.getEntity(t):void 0,i=e?Number(e.state):NaN;if(!e||"unavailable"===e.state||"unknown"===e.state||Number.isNaN(i))return this._offline();const a=this._num(e.attributes.kcal_target),s=Se(i,a),r=Number.isFinite(a)&&a>0,n=r?a-i:NaN,o=r?n>0?`${_i.format(Math.round(n))} kcal kvar`:0===n?"Målet nått":`${_i.format(Math.round(-n))} över målet`:null,l=this._num(e.attributes.protein_g),h=this._num(e.attributes.protein_target_g),c=Number.isFinite(l)&&Number.isFinite(h)&&h>0,d=c?Math.max(0,Math.min(100,l/h*100)):0,p=this._meals(e),u=e.attributes.date,g="string"!=typeof u||Number.isNaN(new Date(`${u}T00:00:00Z`).getTime())?"":Ei.format(new Date(`${u}T00:00:00Z`));return V`
      <div class="page">
        <div class="header">
          ...
          <section class="card">
            <div class="ring-wrap">
              ...
              <div class="ring-center">
                <span class="kc-num">${_i.format(Math.round(i))}</span>
                <span class="kc-target">
                  ${r?`/ ${_i.format(a)} kcal`:"kcal"}
                </span>
              </div>
            </div>
            ...
            <div class="meals">
              ...
              ${p.length?p.map(t=>V`
                      <div class="meal">
                        ...
                        <span class="meal-kcal">${_i.format(Math.round(t.kcal))} kcal</span>
                      </div>
    ...
    `],t([gt({attribute:!1})],Mi.prototype,"config",void 0),customElements.define("hub-kcal-page",Mi);const Si=new Intl.NumberFormat("sv-SE"),Ai=new Intl.DateTimeFormat("sv-SE",{weekday:"long",day:"numeric",month:"long",timeZone:"UTC"});class Ti extends vt{constructor(){super(...arguments),this._openDate=null,this._confirming=!1}_model(){const t=this.config?.kcal?.planner_entity;if(!t)return null;const e=this.getEntity(t);return e&&"unavailable"!==e.state&&"unknown"!==e.state?Be(e.attributes):null}_confirm(t){this._confirming||t.confirmed||0===t.meals.length||(this._confirming=!0,this.callService("rest_command","kcal_confirm_day",{date:t.date}),window.setTimeout(()=>{const t=this.config?.kcal?.planner_entity;t&&this.callService("homeassistant","update_entity",void 0,t),this._confirming=!1,this._openDate=null},1500))}_dayPopup(t){const e=t.days.find(t=>t.date===this._openDate);if(!e)return q;const i=Ai.format(new Date(`${e.date}T00:00:00Z`)),a=!e.confirmed&&e.meals.some(t=>!t.logged);return V`
      <div class="scrim" @click=${()=>this._openDate=null}>
        <div class="popup" @click=${t=>t.stopPropagation()}>
          <h2 class="popup-title">${i}</h2>
          <div class="popup-sub">
            ${e.day_type} · ${Si.format(e.total_kcal)} / ${Si.format(e.target_kcal)} kcal
            ${e.confirmed?" · bekräftad ✓":""}
          </div>
          ${e.meals.map(t=>V`
              <div class="pm">
                <div>
                  <span class="pm-slot">${Te[t.slot]}${t.logged?" · loggad":""}</span>
                  <span class="pm-name">${t.name}</span>
                </div>
                <span class="pm-macro">
                  ${Si.format(t.kcal)} kcal<br />
                  P ${Si.format(t.protein)} · F ${Si.format(t.fat)} · K ${Si.format(t.carbs)}
                </span>
              </div>
            `)}
          ${0===e.meals.length?V`<div class="empty-day">Inget planerat.</div>`:q}
          <div class="popup-actions">
            ${e.confirmed?V`<span class="confirmed-note">Dagen är låst ✓</span>`:q}
    ...
    `}_dayCard(t,e){const i=Pe.filter(e=>t.meals.some(t=>t.slot===e));return V`
      <button
      ...
      >
        <div class="day-head">
          ...
          ${t.confirmed?V`<span class="lock">✓</span>`:q}
          <span class="type-chip ${t.day_type}">${a=t.day_type,Ne[a]??"·"}</span>
        </div>
        <div class="slots">
          ${0===i.length?V`<span class="empty-day">—</span>`:q}
          ${i.map(e=>V`
              <div>
                <span class="slot-label">${Te[e]}</span>
                ${t.meals.filter(t=>t.slot===e).map(t=>V`
                      <div class="meal">
                        <span class="meal-name${t.logged?" logged":""}">${t.name}</span>
                        <span class="meal-kcal">${Si.format(t.kcal)} kcal</span>
                      </div>
                    `)}
              </div>
            `)}
        ...
        <div class="day-foot">
          ${t.meals.length>0?V`${Si.format(t.total_kcal)} / ${Si.format(t.target_kcal)}
              ${t.kcal_ok&&t.protein_ok?q:V`<span class="warn"> ⚠</span>`}`:V`&nbsp;`}
        </div>
      </button>
    ...
    `:V`<div class="page"><div class="offline">Vecka · offline</div></div>`}}Ti.styles=[Tt,n`
      :host {
        display: block;
        height: 100%;
    ...
    `],t([gt({attribute:!1})],Ti.prototype,"config",void 0),t([bt()],Ti.prototype,"_openDate",void 0),t([bt()],Ti.prototype,"_confirming",void 0),customElements.define("hub-planner-page",Ti);const Pi=X`
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
    <path d="M6 6l12 12M18 6L6 18"></path>
  </svg>
`;class Ni extends vt{constructor(){super(...arguments),this.room=null,this._onScrim=t=>{t.target===t.currentTarget&&this._close()}}_close(){this.dispatchEvent(new CustomEvent("hub-popup-close",{bubbles:!0,composed:!0}))}_activateScene(t){this.callService("scene","turn_on",void 0,t)}render(){if(!this.room||!this.hass)return V``;const t=this.room;return V`
    ...
    `],t([gt({attribute:!1})],Ni.prototype,"room",void 0),customElements.define("hub-room-popup",Ni);const zi=X`
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
    <path d="M6 6l12 12M18 6L6 18"></path>
  </svg>
`;class Fi extends vt{constructor(){super(...arguments),this.entity="",this.name="",this._onScrim=t=>{t.target===t.currentTarget&&this._close()}}_close(){this.dispatchEvent(new CustomEvent("hub-popup-close",{bubbles:!0,composed:!0}))}render(){return this.entity&&this.hass?V`
      <div class="scrim" @click=${this._onScrim}>
    ...
    `],t([gt()],Fi.prototype,"entity",void 0),t([gt()],Fi.prototype,"name",void 0),customElements.define("hub-light-popup",Fi);const Di=X`
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
    <path d="M6 6l12 12M18 6L6 18"></path>
  </svg>
`,Bi=new Set(["EXPECTED","ATSTOP"]);class Oi extends vt{constructor(){super(...arguments),this._onScrim=t=>{t.target===t.currentTarget&&this._close()}}_close(){this.dispatchEvent(new CustomEvent("hub-popup-close",{bubbles:!0,composed:!0}))}_departures(t,e,i){if(!t)return[];const a=this.getEntity(t);return Le(a?.attributes.departures??[],e,i).slice(0,6)}_depRow(t){const e=function(t){return"string"==typeof t.state&&t.state.length>0&&!Bi.has(t.state)}(t),i=Ve(t.expected??t.scheduled)??"–";return V`
      <div class="dep-row">
    ...
    `}_storSection(t){return 0===t.length?q:V`
      <div class="section">
        ...
        ${t.map(t=>V`
            <div class="stor">
              ...
              ${t.details?V`<div class="stor-details">${t.details}</div>`:q}
              ${t.scope?V`<div class="stor-scope">Berör: ${t.scope}</div>`:q}
            </div>
          `)}
      </div>
    `}render(){if(!this.hass||!this.config)return V``;const t=this.config,e=this._departures(t.departures?.list_entity,"43",""),i=t.transit?.bus,a=i?this._departures(i.entity,i.line,i.exclude_destination):[],s=t.disturbances_entity?this.getEntity(t.disturbances_entity):void 0,r=s&&"unavailable"!==s.state&&"unknown"!==s.state?Re(s.attributes.deviations):[];return V`
      <div class="scrim" @click=${this._onScrim}>
        <div class="card" role="dialog" aria-label="Resor och störningar">
          <div class="head">
            <span class="title">Resor & störningar</span>
            <button class="close" aria-label="Stäng" @click=${this._close}>${Di}</button>
          </div>
    ...
    `],t([gt({attribute:!1})],Oi.prototype,"config",void 0),customElements.define("hub-transit-popup",Oi);const ji=X`
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
    <path d="M6 6l12 12M18 6L6 18"></path>
  </svg>
`,Ii=new Intl.DateTimeFormat("sv-SE",{weekday:"short"});class Hi extends vt{constructor(){super(...arguments),this._loc=0,this._hours=[],this._days=[],this._bgOn=zt(),this._loadedFor="",this._onScrim=t=>{t.target===t.currentTarget&&this._close()}}get _locations(){const t=this.config;return t?.weather_locations?.length?t.weather_locations:t?.weather_entity?[{entity:t.weather_entity,name:"Hem"}]:[]}updated(t){const e=this._locations[this._loc]?.entity;e&&this.hass&&(t.has("hass")||t.has("config"))&&this._loadedFor!==e&&this._load(e)}async _load(t){this._loadedFor=t;const[e,i]=await Promise.all([ve(this.hass,t,"hourly"),ve(this.hass,t,"daily")]);this._loadedFor===t&&(this._hours=e?Wt(e):[],this._days=i?qt(i):[])}_pickLoc(t){if(t===this._loc)return;this._loc=t,this._hours=[],this._days=[];const e=this._locations[t]?.entity;e&&this._load(e)}_close(){this.dispatchEvent(new CustomEvent("hub-popup-close",{bubbles:!0,composed:!0}))}_toggleBg(){this._bgOn=!this._bgOn,Ft(this._bgOn),this.dispatchEvent(new CustomEvent("hub-weather-bg-toggle",{detail:{on:this._bgOn},bubbles:!0,composed:!0}))}get _isNight(){return"below_horizon"===this.hass?.states["sun.sun"]?.state}_hero(){const t=this.getEntity(this._locations[this._loc]?.entity??"");if(!t)return q;const e=t.attributes.temperature,i=t.attributes.apparent_temperature,a=t.attributes.wind_speed,s=t.attributes.wind_speed_unit??"km/h";return V`
      <div class="hero">
        ${be(t.state,this._isNight)}
        <span class="hero-temp">${"number"==typeof e?Math.round(e):"–"}°</span>
        <span class="hero-meta">
          <span class="hero-cond">${this.hass.formatEntityState(t)}</span>
          ${"number"==typeof i?V`<span>Känns som ${Math.round(i)}°</span>`:q}
          ${"number"==typeof a?V`<span>Vind ${Math.round(a)} ${s}</span>`:q}
        </span>
      </div>
    ...
    `}_daily(){const t=this._days.slice(0,7),e=function(t){if(0===t.length)return null;let e=1/0,i=-1/0;for(const a of t)null!==a.low&&a.low<e&&(e=a.low),a.high>i&&(i=a.high),a.high<e&&(e=Math.min(e,a.high));return Number.isFinite(e)&&Number.isFinite(i)?{min:e,max:i}:null}(t),i=e?Math.max(e.max-e.min,1):1;return V`
      <div class="section">
        ...
        ${t.length&&e?t.map((t,a)=>{const s=t.low??t.high,r=(s-e.min)/i*100,n=Math.max((t.high-s)/i*100,4);return V`
                <div class="day-row">
                  <span class="day-name">${0===a?"Idag":function(t){return t.length?t.charAt(0).toUpperCase()+t.slice(1):t}(Ii.format(new Date(t.ts)))}</span>
                  ${be(t.condition,!1)}
    ...
    `],t([gt({attribute:!1})],Hi.prototype,"config",void 0),t([bt()],Hi.prototype,"_loc",void 0),t([bt()],Hi.prototype,"_hours",void 0),t([bt()],Hi.prototype,"_days",void 0),t([bt()],Hi.prototype,"_bgOn",void 0),customElements.define("hub-weather-popup",Hi);const Li={hem:{label:"Hem",icon:"home",tone:"neutral"},ljus:{label:"Ljus",icon:"lamp",tone:"amber"},media:{label:"Media",icon:"note",tone:"teal"},energi:{label:"Energi",icon:"bolt",tone:"green"},kcal:{label:"Kcal",icon:"ring",tone:"lavender"},vecka:{label:"Vecka",icon:"calendar",tone:"lavender"}};class Ri extends ht{constructor(){super(...arguments),this.pages=[],this.active=0}_select(t){this.dispatchEvent(new CustomEvent("hub-goto-page",{detail:{page:t},bubbles:!0,composed:!0}))}render(){return V`
      <nav>
        <div class="rail"></div>
        <div class="items">
          ${this.pages.map((t,e)=>{const i=function(t){const e=Li[t];return e?{id:t,...e}:{id:t,label:t.charAt(0).toUpperCase()+t.slice(1),icon:"",tone:"neutral"}}(t),a=e===this.active,s=It[i.icon];return V`
              <button
                class="item tone-${i.tone} ${a?"active":""}"
                aria-label=${i.label}
                aria-current=${a?"page":q}
                @click=${()=>this._select(t)}
              >
                <span class="pill">
                  ${s?V`<span class="icon">${s}</span>`:q}
                </span>
    ...
    `],t([gt({attribute:!1})],Ri.prototype,"pages",void 0),t([gt({type:Number})],Ri.prototype,"active",void 0),customElements.define("hub-nav-bar",Ri);const Ui=["hem","ljus","media","energi","kcal","vecka"],Vi={hem:"Hem",ljus:"Ljus",media:"Media",energi:"Energi",kcal:"Kcal",vecka:"Vecka"};const Xi=["auto","dag","natt"];let Wi=0;class qi extends vt{constructor(){super(...arguments),this.theme="natt",this.kiosk=new URLSearchParams(location.search).has("kiosk"),this._page=0,this._dragX=0,this._openRoom=null,this._openLight=null,this._openTransit=!1,this._openWeather=!1,this._weatherBgOn=zt(),this._override=function(){const t=localStorage.getItem(Pt);return"natt"===t||"dag"===t?t:"auto"}(),this._pointerActive=!1,this._dragging=!1,this._startX=0,this._startY=0,this._lastX=0,this._lastT=0,this._velocity=0,this._onRoomOpen=t=>{const e=t.detail?.roomId;this._openRoom=this._cfg?.rooms?.find(t=>t.id===e)??null},this._onLightOpen=t=>{const e=t.detail;this._openLight=e?.entity?{entity:e.entity,name:e.name??e.entity}:null},this._onGotoPage=t=>{const e=t.detail?.page;e&&this.goToPage(e)},this._onTransitOpen=()=>{this._openTransit=!0},this._onWeatherOpen=()=>{this._openWeather=!0},this._onWeatherBgToggle=t=>{this._weatherBgOn=t.detail?.on??zt(),Ft(this._weatherBgOn)},this._onPopupClose=()=>{this._openRoom=null,this._openLight=null,this._openTransit=!1,this._openWeather=!1},this._onAnyInteraction=()=>{this._resetIdle()},this._onPointerDown=t=>{this._pointerActive=!0,this._dragging=!1,this._startX=t.clientX,this._startY=t.clientY,this._lastX=t.clientX,this._lastT=t.timeStamp,this._velocity=0,this._dragX=0},this._onPointerMove=t=>{if(!this._pointerActive)return;const e=t.clientX-this._startX,i=t.clientY-this._startY;if(!this._dragging){if(!Ot(e)&&!Ot(i))return;if(!function(t,e){return Math.abs(t)>Math.abs(e)}(e,i))return void(this._pointerActive=!1);this._dragging=!0,t.currentTarget.setPointerCapture?.(t.pointerId),this._lastX=t.clientX,this._lastT=t.timeStamp}const a=t.timeStamp-this._lastT;a>0&&(this._velocity=(t.clientX-this._lastX)/a),this._lastX=t.clientX,this._lastT=t.timeStamp,this._dragX=e},this._onPointerUp=t=>{if(!this._pointerActive)return;const e=this._dragging;if(this._pointerActive=!1,this._dragging=!1,e){t.currentTarget.releasePointerCapture?.(t.pointerId);const e=this.clientWidth||window.innerWidth;this._page=function(t,e,i,a,s){const r=.2*e,n=Math.abs(i)>.5;let o=a;return t<-r||n&&i<-.5?o=a+1:(t>r||n&&i>.5)&&(o=a-1),Math.max(0,Math.min(s-1,o))}(this._dragX,e,this._velocity,this._page,this._pages.length),Wi=this._page}this._dragX=0,this._velocity=0}}setConfig(t){super.setConfig(t)}get _cfg(){return this._config}get _pages(){return this._cfg?.pages??Ui}connectedCallback(){super.connectedCallback(),function(){if(document.getElementById("glass-hub-fonts"))return;const t=document.createElement("style");t.id="glass-hub-fonts",t.textContent="\n@font-face{font-family:'Outfit';src:url('/local/glass-cards/fonts/outfit-variable.woff2') format('woff2-variations');font-weight:100 900;font-display:swap;}\n@font-face{font-family:'Inter';src:url('/local/glass-cards/fonts/inter-variable.woff2') format('woff2-variations');font-weight:100 900;font-display:swap;}\n",document.head.appendChild(t)}(),this._applyTheme(),this._page=Wi,this._resetIdle(),this._startKioskDrawerShim(),function(){const t=new URLSearchParams(location.search).get("weather");t&&(Dt=t),window.__hubForceWeather=t=>{Dt=t,window.dispatchEvent(new CustomEvent("hub-weather-force"))}}(),this.addEventListener("pointerdown",this._onAnyInteraction),this.addEventListener("hub-room-open",this._onRoomOpen),this.addEventListener("hub-light-open",this._onLightOpen),this.addEventListener("hub-transit-open",this._onTransitOpen),this.addEventListener("hub-goto-page",this._onGotoPage),this.addEventListener("hub-popup-close",this._onPopupClose),this.addEventListener("hub-weather-open",this._onWeatherOpen),this.addEventListener("hub-weather-bg-toggle",this._onWeatherBgToggle)}disconnectedCallback(){super.disconnectedCallback(),this._clearIdle(),void 0!==this._kioskTimer&&(clearInterval(this._kioskTimer),this._kioskTimer=void 0),this.removeEventListener("pointerdown",this._onAnyInteraction),this.removeEventListener("hub-room-open",this._onRoomOpen),this.removeEventListener("hub-light-open",this._onLightOpen),this.removeEventListener("hub-transit-open",this._onTransitOpen),this.removeEventListener("hub-goto-page",this._onGotoPage),this.removeEventListener("hub-popup-close",this._onPopupClose),this.removeEventListener("hub-weather-open",this._onWeatherOpen),this.removeEventListener("hub-weather-bg-toggle",this._onWeatherBgToggle)}willUpdate(t){t.has("hass")&&this._applyTheme()}goToPage(t){const e=this._pages.indexOf(t);e>=0&&(this._page=e,Wi=e,this._dragX=0)}_applyTheme(){const t=this.hass?.states["sun.sun"]?.attributes?.elevation,e="number"==typeof t?t:null;this.theme=function(t,e,i=4){return"auto"!==e?e:null===t?"natt":t>i?"dag":"natt"}(e,this._override,this._cfg?.day_elevation??4)}_cycleTheme(){const t=Xi.indexOf(this._override);this._override=Xi[(t+1)%Xi.length],function(t){localStorage.setItem(Pt,t)}(this._override),this._applyTheme()}_toggleKiosk(){const t=new URLSearchParams(location.search);t.has("kiosk")?t.delete("kiosk"):t.set("kiosk","true");const e=t.toString();location.assign(location.pathname+(e?`?${e}`:""))}_resetIdle(){this._clearIdle();const t=this._cfg?.idle_return_s??120;this._idleTimer=window.setTimeout(()=>{0!==this._page&&this.goToPage(this._pages[0])},1e3*t)}_clearIdle(){void 0!==this._idleTimer&&(clearTimeout(this._idleTimer),this._idleTimer=void 0)}_startKioskDrawerShim(){if(!new URLSearchParams(location.search).has("kiosk"))return;const t=Date.now(),e=()=>{const t=document.querySelector("home-assistant")?.shadowRoot?.querySelector("home-assistant-main");if(!t)return!1;t.style.setProperty("--mdc-drawer-width","0px");const e=t.shadowRoot?.querySelector("ha-drawer");return e?.style.setProperty("--mdc-drawer-width","0px"),!0};e()||(this._kioskTimer=window.setInterval(()=>{(e()||Date.now()-t>5e3)&&(clearInterval(this._kioskTimer),this._kioskTimer=void 0)},250))}_themeGlyph(){return"auto"===this._override?V`<span class="glyph-auto">A</span>`:"dag"===this._override?X`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"
                      ...
                      ></hub-energy-page>`:"media"===e?V`<hub-media-page
                        ...
                        ></hub-media-page>`:"kcal"===e?V`<hub-kcal-page
                          ...
                          ></hub-kcal-page>`:"vecka"===e?V`<hub-planner-page
                              .hass=${this.hass}
                              .config=${this._cfg}
                            ></hub-planner-page>`:V`<h1 class="page-placeholder">${function(t){return Vi[t]??t.charAt(0).toUpperCase()+t.slice(1)}(e)}</h1>`}
            </section>
          `)}
      </div>
      ...
      <hub-nav-bar .pages=${t} .active=${this._page}>
        ...
        >
          ${this.kiosk?It.compress:It.expand}
        </button>
        <button
          slot="controls"
          class="theme-toggle"
