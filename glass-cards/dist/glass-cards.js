...
`,Ut={lamp:Rt(X`
  ...
  `)};class Bt extends ct{constructor(){super(...arguments),this.icon="",this.label="",this.tone="neutral",this.active=!1}render(){const t=Ut[this.icon];return V`
    ...
    `}}function Zt(t){const e=Date.parse(t.expected??t.scheduled??"");return Number.isNaN(e)?Number.POSITIVE_INFINITY:e}Kt.styles=[Tt,n`
      :host {
        display: block;
        height: 100%;
      }
    ...
    `}render(){if(!this.hass||!this.config)return V``;const t=this.config,e=function(t,e){let i=0,a=0;for(const s of t.rooms??[])for(const t of s.lights){const s=e[t.entity];ae(s)&&(a+=1,"on"===s.state&&(i+=1))}return{on:i,total:a}}(t,this.hass.states);return V`
      <div class="page">
        <div class="header">
          <div class="heading">
    ...
    `],t([gt({attribute:!1})],he.prototype,"player",void 0),t([gt({attribute:!1})],he.prototype,"groupMaster",void 0),t([bt()],he.prototype,"_drag",void 0),customElements.define("hub-volume-row",he);const de=new Set(["off","unavailable","unknown","standby","idle"]);function pe(t){const e=Number.isFinite(t)&&t>0?t:0,i=Math.floor(e/60),a=Math.floor(e%60);return`${i}:${String(a).padStart(2,"0")}`}class ue extends vt{constructor(){super(...arguments),this._sel=null,this._rgb=null,this._now=Date.now()}connectedCallback(){super.connectedCallback(),this._interval=window.setInterval(()=>{this._now=Date.now()},1e3)}disconnectedCallback(){super.disconnectedCallback(),void 0!==this._interval&&(clearInterval(this._interval),this._interval=void 0)}get _players(){return this.config?.media_players??[]}_selId(){if(this._sel)return this._sel;const t=Xt(this.hass?.states??{},this._players);return t?.entity.entity_id??this._players[0]?.entity??null}_theme(){const t=this.getRootNode()?.host;return"dag"===t?.getAttribute("data-theme")?"dag":"natt"}updated(t){const e=this._selId(),i=e?this.hass?.states[e]?.attributes.entity_picture:void 0;i!==this._pic&&(this._pic=i,i?le(i).then(t=>{this._pic===i&&(this._rgb=t)}):this._rgb=null)}_transport(t,e){this.callService("media_player",t,void 0,e)}_hero(t,e){const i="playing"===t.state,a=t.attributes.media_title||e,s=t.attributes.media_artist||e,r=t.attributes.entity_picture,n="number"==typeof t.attributes.media_duration?t.attributes.media_duration:0,o=Gt(t,this._now),l=o/100*n,c=t.entity_id;return V`
      <div class="hero">
    ...
    `],t([gt({attribute:!1})],ue.prototype,"config",void 0),t([bt()],ue.prototype,"_sel",void 0),t([bt()],ue.prototype,"_rgb",void 0),t([bt()],ue.prototype,"_now",void 0),customElements.define("hub-media-page",ue);let ge=0;class be extends ct{constructor(){super(...arguments),this.points=[],this.stroke="--hub-lavender",this.width=560,this.height=130,this._gid="hub-spark-"+ge++}render(){const t=function(t,e,i,a=.1){const s=t.length;if(0===s)return[];if(1===s)return[{x:e,y:i/2}];const r=t.map(t=>t.value),n=Math.min(...r),o=Math.max(...r)-n,l=n-o*a,c=o*(1+2*a);return t.map((t,a)=>({x:a/(s-1)*e,y:o<=0?i/2:i-(t.value-l)/c*i}))}(this.points,this.width,this.height);if(0===t.length)return V``;const e=t.map(t=>`${t.x.toFixed(2)},${t.y.toFixed(2)}`).join(" "),i=t[t.length-1],a=t[0],s=t.length>=2,r=`${e} ${i.x.toFixed(2)},${this.height} ${a.x.toFixed(2)},${this.height}`;return V`
  ...
  `,t([gt({attribute:!1})],be.prototype,"points",void 0),t([gt()],be.prototype,"stroke",void 0),t([gt({type:Number})],be.prototype,"width",void 0),t([gt({type:Number})],be.prototype,"height",void 0),customElements.define("hub-sparkline",be);const me=new Intl.NumberFormat("sv-SE"),ve=new Intl.NumberFormat("sv-SE",{minimumFractionDigits:1,maximumFractionDigits:1}),fe=new Intl.NumberFormat("sv-SE",{maximumFractionDigits:1}),xe=new Intl.DateTimeFormat("sv-SE",{day:"numeric",month:"short",timeZone:"UTC"}),ye=new Intl.DateTimeFormat("sv-SE",{weekday:"long",day:"numeric",month:"long",timeZone:"UTC"});function _e(t){if(!t)return"";const e=new Date(`${t}T00:00:00Z`);return Number.isNaN(e.getTime())?"":xe.format(e).replace(/\.$/,"")}class we extends vt{_meals(t){const e=t.attributes.meals;return Array.isArray(e)?e.filter(t=>!!t&&"object"==typeof t).map(t=>({name:"string"==typeof t.name?t.name:"",kcal:"number"==typeof t.kcal?t.kcal:Number(t.kcal)||0})).filter(t=>t.name):[]}_num(t){return"number"==typeof t?t:NaN}_offline(){return V`
      <div class="page">
        <div class="offline">
    ...
    `}_weightCard(){const t=this.config.kcal?.forecast_entity,e=t?this.getEntity(t):void 0,i=e?Number(e.state):NaN;if(!e||"unavailable"===e.state||"unknown"===e.state||Number.isNaN(i))return V`
      ...
      `;const a=e.attributes.weight_trend,s=Array.isArray(a)?a.filter(t=>!!t&&"object"==typeof t).map(t=>({date:String(t.date??""),value:Number(t.kg)})).filter(t=>Number.isFinite(t.value)):[],r=function(t){if(t.length<2)return null;const e=new Date(`${t[0].date}T00:00:00Z`).getTime(),i=new Date(`${t[t.length-1].date}T00:00:00Z`).getTime();return Number.isNaN(e)||Number.isNaN(i)?null:Math.round((i-e)/864e5)}(s),n=s.length>=2?s[s.length-1].value-s[0].value:null,o=null===n||null===r?null:`${n<0?"−":n>0?"+":""}${ve.format(Math.abs(n))} kg på ${r} ${1===r?"dag":"dagar"}`,l=e.attributes.forecast,c=l&&"object"==typeof l?l:null,h=c?function(t){const e="number"==typeof t.goal_kg?`Mål ${fe.format(t.goal_kg)} kg`:"",i=t.eta?_e(t.eta):"",a=t.eta_early&&t.eta_late?`${_e(t.eta_early)}–${_e(t.eta_late)}`:"";return[e,i?`ETA ${i}${a?` (${a})`:""}`:""].filter(Boolean).join(" · ")}(c):"",d=!!c?.on_track;return V`
      <section class="card">
        <span class="w-eyebrow">Vikt</span>
        <div class="w-num-row">
          <span class="w-num">${ve.format(i)}</span>
    ...
    `}render(){if(!this.hass||!this.config)return V``;const t=this.config.kcal?.today_entity,e=t?this.getEntity(t):void 0,i=e?Number(e.state):NaN;if(!e||"unavailable"===e.state||"unknown"===e.state||Number.isNaN(i))return this._offline();const a=this._num(e.attributes.kcal_target),s=Yt(i,a),r=Number.isFinite(a)&&a>0,n=r?a-i:NaN,o=r?n>0?`${me.format(Math.round(n))} kcal kvar`:0===n?"Målet nått":`${me.format(Math.round(-n))} över målet`:null,l=this._num(e.attributes.protein_g),c=this._num(e.attributes.protein_target_g),h=Number.isFinite(l)&&Number.isFinite(c)&&c>0,d=h?Math.max(0,Math.min(100,l/c*100)):0,p=this._meals(e),u=e.attributes.date,g="string"!=typeof u||Number.isNaN(new Date(`${u}T00:00:00Z`).getTime())?"":ye.format(new Date(`${u}T00:00:00Z`));return V`
      <div class="page">
        <div class="header">
          ...
          <section class="card">
            <div class="ring-wrap">
              ...
              <div class="ring-center">
                <span class="kc-num">${me.format(Math.round(i))}</span>
                <span class="kc-target">
                  ${r?`/ ${me.format(a)} kcal`:"kcal"}
                </span>
              </div>
            </div>
            ...
            <div class="meals">
              ...
              ${p.length?p.map(t=>V`
                      <div class="meal">
                        ...
                        <span class="meal-kcal">${me.format(Math.round(t.kcal))} kcal</span>
                      </div>
    ...
    `],t([gt({attribute:!1})],we.prototype,"config",void 0),customElements.define("hub-kcal-page",we);const $e=X`
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
    <path d="M6 6l12 12M18 6L6 18"></path>
  </svg>
`;class ke extends vt{constructor(){super(...arguments),this.room=null,this._onScrim=t=>{t.target===t.currentTarget&&this._close()}}_close(){this.dispatchEvent(new CustomEvent("hub-popup-close",{bubbles:!0,composed:!0}))}render(){if(!this.room||!this.hass)return V``;const t=this.room;return V`
    ...
    `],t([gt({attribute:!1})],ke.prototype,"room",void 0),customElements.define("hub-room-popup",ke);const Ce={hem:{label:"Hem",icon:"home",tone:"neutral"},ljus:{label:"Ljus",icon:"lamp",tone:"amber"},media:{label:"Media",icon:"note",tone:"teal"},energi:{label:"Energi",icon:"bolt",tone:"green"},kcal:{label:"Kcal",icon:"ring",tone:"lavender"}};class Ee extends ct{constructor(){super(...arguments),this.pages=[],this.active=0}_select(t){this.dispatchEvent(new CustomEvent("hub-goto-page",{detail:{page:t},bubbles:!0,composed:!0}))}render(){return V`
      <nav>
        ${this.pages.map((t,e)=>{const i=function(t){const e=Ce[t];return e?{id:t,...e}:{id:t,label:t.charAt(0).toUpperCase()+t.slice(1),icon:"",tone:"neutral"}}(t),a=e===this.active,s=Ut[i.icon];return V`
            <button
                    ...
                    ></hub-lights-page>`:"energi"===t?V`<hub-energy-page
                      ...
                      ></hub-energy-page>`:"media"===t?V`<hub-media-page
                        ...
                        ></hub-media-page>`:"kcal"===t?V`<hub-kcal-page
                            .hass=${this.hass}
                            .config=${this._cfg}
                          ></hub-kcal-page>`:V`<h1 class="page-placeholder">${function(t){return Ae[t]??t.charAt(0).toUpperCase()+t.slice(1)}(t)}</h1>`}
            </section>
          `)}
      </div>
